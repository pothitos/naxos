...
void
ConstrXlessthanY::LocalArcCons (QueueItem& Qitem)
{
        if (VarX == Qitem.getVarFired()) {
                ...
                assert_exc(VarY == Qitem.getVarFired(),  "ConstrXlessthanY::LocalArcCons: Wrong getVarFired");
                //while ( VarX->max() >= VarY->max() )   {
                //	if ( !VarX->removeSingle( VarX->max(), this ) )
                //		return;	// ... to avoid an infinite loop
                //}
...
void
ConstrXlesseqthanY::ArcCons (void)
{
        //while ( VarX->max() > VarY->max() )   {
        //	if ( !VarX->removeSingle( VarX->max(), this ) )
        //		return;	// ... to avoid an infinite loop
...
void
ConstrXlesseqthanY::LocalArcCons (QueueItem& Qitem)
{
        if (VarX == Qitem.getVarFired()) {
                //while ( VarX->min() > VarY->min() )   {
                //	if ( !VarY->removeSingle( VarY->min(), this ) )
                ...
                assert_exc(VarY == Qitem.getVarFired(),  "ConstrXlesseqthanY::LocalArcCons: Wrong getVarFired");
                //while ( VarX->max() > VarY->max() )   {
                //	if ( !VarX->removeSingle( VarX->max(), this ) )
                //		return;	// ... to avoid an infinite loop
                //}
...
namespace {
void
MakeBoolean (NsIntVar *VarX, const NsConstraint *constraint)
{
        //while ( VarX->min() < 0 )   {
...
void
ConstrMetaXeqYlessthanC::ArcCons (void)
{
        ...
        if (VarX->isBound()) {
                if (VarX->value() == 0) {
                        ...
                        assert_exc(VarX->value() == 1,  "ConstrMetaXeqYlessthanC::ArcCons: Wrong `VarX->value()'");
                        //while ( VarY->max() >= C )   {
...
void
ConstrMetaXeqYlessthanC::LocalArcCons (QueueItem& Qitem)
{
        if (VarX == Qitem.getVarFired()) {
                if ( VarX->isBound() ) {
                        if (VarX->value() == 0) {
                                ...
                                assert_exc(VarX->value() == 1,
                                           "ConstrMetaXeqYlessthanC::LocalArcCons: Wrong `VarX->value()'");
                                //while ( VarY->max() >= C )   {
                                //	if ( !VarY->removeSingle( VarY->max(), this ) )
                                //		return;	// ... to avoid an infinite loop
                                //}
                ...
                assert_exc(VarY == Qitem.getVarFired(),  "ConstrMetaXeqYlessthanC::LocalArcCons: Wrong getVarFired");
                if (VarX->contains(0)  &&  VarY->max() < C)
                        VarX->removeSingle( 0, this );
                if (VarX->contains(1)  &&  VarY->min() >= C)
                        VarX->removeSingle( 1, this );
...
void
ConstrMetaXeqYlessthanZ::ArcCons (void)
{
        MakeBoolean(VarX, this);
        ...
        if (VarX->isBound()) {
                if (VarX->value() == 0) {
                        ...
                        //assert_exc(VarX->value() == 1,  "ConstrMetaXeqYlessthanZ::ArcCons: Wrong `VarX->value()'");
...
void
ConstrMetaXeqYlessthanZ::LocalArcCons (QueueItem& Qitem)
{
        if (VarX == Qitem.getVarFired()) {
                if ( VarX->isBound() ) {
                        if (VarX->value() == 0) {
                                ...
                                assert_exc(VarX->value() == 1,
                                           "ConstrMetaXeqYlessthanZ::LocalArcCons: Wrong `VarX->value()'");
                                //while ( VarY->max() >= VarZ->max() )   {
                                //	if ( !VarY->removeSingle( VarY->max(), this ) )
                                //		return;	// ... to avoid an infinite loop
                                //}
                ...
                if (VarY == Qitem.getVarFired()) {
                        ...
                        assert_exc(VarZ == Qitem.getVarFired(),
                                   "ConstrMetaXeqYlessthanZ::LocalArcCons: Wrong getVarFired");
                        if ( VarX->isBound() ) {
                                if (VarX->value() == 0) {
                                        //while ( VarZ->min() > VarY->min() )   {
                                        //	if ( !VarY->removeSingle( VarY->min(), this ) )
...
void
ConstrMetaXeqYlesseqthanC::ArcCons (void)
{
        MakeBoolean(VarX, this);
        if (VarX->contains(0)  &&  VarY->max() <= C)
                VarX->removeSingle( 0, this );
        ...
        if (VarX->isBound()) {
                if (VarX->value() == 0) {
                        ...
                        assert_exc(VarX->value() == 1,  "ConstrMetaXeqYlesseqthanC::ArcCons: Wrong `VarX->value()'");
                        //while ( VarY->max() > C )   {
                        //	if ( !VarY->removeSingle( VarY->max(), this ) )
                        //		return;	// ... to avoid an infinite loop
                        //}
...
void
ConstrMetaXeqYlesseqthanC::LocalArcCons (QueueItem& Qitem)
{
        if (VarX == Qitem.getVarFired()) {
                if ( VarX->isBound() ) {
                        if (VarX->value() == 0) {
                                ...
                                assert_exc(VarX->value() == 1,
                                           "ConstrMetaXeqYlesseqthanC::LocalArcCons: Wrong `VarX->value()'");
                                //while ( VarY->max() > C )   {
                                //	if ( !VarY->removeSingle( VarY->max(), this ) )
                                //		return;	// ... to avoid an infinite loop
                                //}
                ...
                assert_exc(VarY == Qitem.getVarFired(),  "ConstrMetaXeqYlesseqthanC::LocalArcCons: Wrong getVarFired");
                if (VarX->contains(0)  &&  VarY->max() <= C)
                        VarX->removeSingle( 0, this );
                if (VarX->contains(1)  &&  VarY->min() > C)
                        VarX->removeSingle( 1, this );
...
void
ConstrMetaXeqYlesseqthanZ::ArcCons (void)
{
        MakeBoolean(VarX, this);
        if (VarX->contains(0)  &&  VarY->max() <= VarZ->min())
                VarX->removeSingle( 0, this );
        ...
        if (VarX->isBound()) {
                if (VarX->value() == 0) {
                        ...
                        assert_exc(VarX->value() == 1,  "ConstrMetaXeqYlesseqthanZ::ArcCons: Wrong `VarX->value()'");
                        //while ( VarY->max() > VarZ->max() )   {
                        //	if ( !VarY->removeSingle( VarY->max(), this ) )
                        //		return;	// ... to avoid an infinite loop
                        //}
...
void
ConstrMetaXeqYlesseqthanZ::LocalArcCons (QueueItem& Qitem)
{
        if (VarX == Qitem.getVarFired()) {
                if ( VarX->isBound() ) {
                        if (VarX->value() == 0) {
                                ...
                                assert_exc(VarX->value() == 1,
                                           "ConstrMetaXeqYlesseqthanZ::LocalArcCons: Wrong `VarX->value()'");
                                //while ( VarY->max() > VarZ->max() )   {
                                //	if ( !VarY->removeSingle( VarY->max(), this ) )
                                //		return;	// ... to avoid an infinite loop
                                //}
                ...
                if (VarY == Qitem.getVarFired()) {
                        if ( VarX->isBound() ) {
                                if (VarX->value() == 0) {
                                        ...
                                        assert_exc(VarX->value() == 1, "ConstrMetaXeqYlesseqthanZ::LocalArcCons: Wrong `VarX->value()'");
                                        //while ( VarY->min() > VarZ->min() )   {
                                        //	if ( !VarZ->removeSingle( VarZ->min(), this ) )
                                        //		return;	// ... to avoid an infinite loop
                                        //}
                        ...
                        assert_exc(VarZ == Qitem.getVarFired(),
                                   "ConstrMetaXeqYlesseqthanZ::LocalArcCons: Wrong getVarFired");
                        if ( VarX->isBound() ) {
                                if (VarX->value() == 0) {
                                        //while ( VarZ->min() >= VarY->min() )   {
                                        //	if ( !VarY->removeSingle( VarY->min(), this ) )
...
void
ConstrMetaXeqYgreaterthanC::ArcCons (void)
{
        MakeBoolean(VarX, this);
        ...
        if (VarX->isBound()) {
                if (VarX->value() == 0) {
                        ...
                        assert_exc(VarX->value() == 1,  "ConstrMetaXeqYgreaterthanC::ArcCons: Wrong `VarX->value()'");
                        //while ( VarY->min() <= C )   {
                        //	if ( !VarY->removeSingle( VarY->min(), this ) )
                        //		return;	// ... to avoid an infinite loop
                        //}
...
void
ConstrMetaXeqYgreaterthanC::LocalArcCons (QueueItem& Qitem)
{
        if (VarX == Qitem.getVarFired()) {
                if ( VarX->isBound() ) {
                        if (VarX->value() == 0) {
                                ...
                                assert_exc(VarX->value() == 1,
                                           "ConstrMetaXeqYgreaterthanC::LocalArcCons: Wrong `VarX->value()'");
                                //while ( VarY->min() <= C )   {
                                //	if ( !VarY->removeSingle( VarY->min(), this ) )
                                //		return;	// ... to avoid an infinite loop
                                //}
                ...
                assert_exc(VarY == Qitem.getVarFired(),  "ConstrMetaXeqYgreaterthanC::LocalArcCons: Wrong getVarFired");
                if (VarX->contains(0)  &&  VarY->min() > C)
                        VarX->removeSingle( 0, this );
                if (VarX->contains(1)  &&  VarY->max() <= C)
                        VarX->removeSingle( 1, this );
...
void
ConstrMetaXeqYgreatereqthanC::ArcCons (void)
{
        MakeBoolean(VarX, this);
        if (VarX->contains(0)  &&  VarY->min() >= C)
                VarX->removeSingle( 0, this );
        ...
        if (VarX->isBound()) {
                if (VarX->value() == 0) {
                        ...
                        assert_exc(VarX->value() == 1,  "ConstrMetaXeqYgreatereqthanC::ArcCons: Wrong `VarX->value()'");
                        //while ( VarY->min() < C )   {
                        //	if ( !VarY->removeSingle( VarY->min(), this ) )
                        //		return;	// ... to avoid an infinite loop
                        //}
...
void
ConstrMetaXeqYgreatereqthanC::LocalArcCons (QueueItem& Qitem)
{
        if (VarX == Qitem.getVarFired()) {
                if ( VarX->isBound() ) {
                        if (VarX->value() == 0) {
                                ...
                                assert_exc(VarX->value() == 1,
                                           "ConstrMetaXeqYgreatereqthanC::LocalArcCons: Wrong `VarX->value()'");
                                //while ( VarY->min() < C )   {
                                //	if ( !VarY->removeSingle( VarY->min(), this ) )
                                //		return;	// ... to avoid an infinite loop
                                //}
                ...
                assert_exc(VarY == Qitem.getVarFired(),
                           "ConstrMetaXeqYgreatereqthanC::LocalArcCons: Wrong getVarFired");
                if (VarX->contains(0)  &&  VarY->min() >= C)
                        VarX->removeSingle( 0, this );
                if (VarX->contains(1)  &&  VarY->max() < C)
                        VarX->removeSingle( 1, this );
...
void
ConstrMetaXeqYeqC::ArcCons (void)
{
        MakeBoolean(VarX, this);
        ...
        if (VarX->isBound()) {
                if (VarX->value() == 0) {
                        ...
                        assert_exc(VarX->value() == 1,  "ConstrMetaXeqYeqC::ArcCons: Wrong `VarX->value()'");
                        //while ( VarY->min() < C )   {
                        //	if ( !VarY->removeSingle( VarY->min(), this ) )
                        //		return;	// ... to avoid an infinite loop
                        //}
...
void
ConstrMetaXeqYeqC::LocalArcCons (QueueItem& Qitem)
{
        if (VarX == Qitem.getVarFired()) {
                if ( VarX->isBound() ) {
                        if (VarX->value() == 0) {
                                ...
                                assert_exc(VarX->value() == 1,
                                           "ConstrMetaXeqYeqC::LocalArcCons: Wrong `VarX->value()'");
                                //while ( VarY->min() < C )   {
                                //	if ( !VarY->removeSingle( VarY->min(), this ) )
                                //		return;	// ... to avoid an infinite loop
                                //}
...
void
ConstrMetaXeqYneqC::ArcCons (void)
{
        MakeBoolean(VarX, this);
        ...
        if (VarX->isBound()) {
                if (VarX->value() == 0) {
                        ...
                        assert_exc(VarX->value() == 1,  "ConstrMetaXeqYneqC::ArcCons: Wrong `VarX->value()'");
                        if ( VarY->contains(C) )
                                VarY->removeSingle( C, this );
                }
        }
...
void
ConstrMetaXeqYneqC::LocalArcCons (QueueItem& Qitem)
{
        if (VarX == Qitem.getVarFired()) {
                if ( VarX->isBound() ) {
                        if (VarX->value() == 0) {
                                ...
                                assert_exc(VarX->value() == 1,
                                           "ConstrMetaXeqYneqC::LocalArcCons: Wrong `VarX->value()'");
                                if ( VarY->contains(C) )
                                        VarY->removeSingle( C, this );
                        }
                }
...
void
ConstrXeqYplusC::LocalArcCons (QueueItem& Qitem)
{
        NsInt  SupportVal;
        if (VarX == Qitem.getVarFired()) {
                ...
                assert_exc(VarY == Qitem.getVarFired(),  "ConstrXeqYplusC::LocalArcCons: Wrong getVarFired");
                SupportVal = Qitem.getW() + C;
...
void
ConstrXeqCminusY::LocalArcCons (QueueItem& Qitem)
{
        NsInt  SupportVal;
        if (VarX == Qitem.getVarFired()) {
                ...
                assert_exc(VarY == Qitem.getVarFired(),  "ConstrXeqCminusY::LocalArcCons: Wrong getVarFired");
                SupportVal = C - Qitem.getW();
                if (VarX->contains( SupportVal ))
                        VarX->removeSingle( SupportVal, this );
        }
...
void
ConstrXeqYtimesC::LocalArcCons (QueueItem& Qitem)
{
        NsInt  SupportVal;
        if (VarX == Qitem.getVarFired()) {
                SupportVal = Qitem.getW() / C;
...
void
ConstrXeqY::LocalArcCons (QueueItem& Qitem)
{
        ...
        if (VarX == Qitem.getVarFired()) {
                ...
                assert_exc(VarY == Qitem.getVarFired(),  "ConstrXeqY::LocalArcCons: Wrong getVarFired");
...
void
ConstrXeqYplusCZspecial::ArcCons (void)
{
...
void
ConstrXeqYplusCZspecial::LocalArcCons (QueueItem& Qitem)
{
        NsIntVar::const_iterator  v, vz;
        NsInt  SupportVal;
        if (VarX == Qitem.getVarFired()) {
                ...
                assert_exc(VarZ == Qitem.getVarFired(),  "ConstrXeqYplusCZspecial::LocalArcCons: Wrong getVarFired");
                //SupportVal = Qitem.getW() * C -1;
                //while ( ( SupportVal = VarX->next(SupportVal) )  <  (Qitem.getW() + 1) * C )
                //	VarX->removeSingle( SupportVal, this );
...
void
ConstrXeqYdivC::LocalArcCons (QueueItem& Qitem)
{
        NsInt  SupportVal;
        if (VarX == Qitem.getVarFired()) {
                //SupportVal = Qitem.getW() * C -1;
                //while ( ( SupportVal = VarY->next(SupportVal) )  <  (Qitem.getW() + 1) * C )
                //	VarY->removeSingle( SupportVal, this );
                VarY->removeRange(Qitem.getW() * C, (Qitem.getW() + 1)*C - 1, this);
        } else {
                assert_exc(VarY == Qitem.getVarFired(),  "ConstrXeqYdivC::LocalArcCons: Wrong getVarFired");
                SupportVal = Qitem.getW() / C;
                if (VarX->contains( SupportVal )) {
...
void
ConstrXeqCdivY::LocalArcCons (QueueItem& Qitem)
{
        NsInt  SupportVal;
        if (VarX == Qitem.getVarFired()) {
                if (Qitem.getW() == 0) {
                        ...
                        //while ( ( SupportVal = VarY->next(SupportVal) )  <=  C / Qitem.getW() )
                        //	VarY->removeSingle( SupportVal, this );
                        VarY->removeRange(C / (Qitem.getW()+1) +1,  C / Qitem.getW(),  this);
                }
        } else {
                assert_exc(VarY == Qitem.getVarFired(),  "ConstrXeqCdivY::LocalArcCons: Wrong getVarFired");
                if (Qitem.getW() == 0)
                        return;
                SupportVal = C / Qitem.getW();
                if (VarX->contains( SupportVal )) {
                        // -C = C/-1
                        if ((SupportVal==-1  &&  NsMINUS_INF == VarY->previous(-C+1))
...
void
naxos::YmodC_min_max (const NsIntVar *VarY, const NsInt C, NsInt& min, NsInt& max)
{
        min = NsPLUS_INF;
        max = NsMINUS_INF;
        NsInt  SupValY, v;
...
void
ConstrXeqYmodC::LocalArcCons (QueueItem& Qitem)
{
        ...
        if (VarX == Qitem.getVarFired()) {
                ...
                assert_exc(VarY == Qitem.getVarFired(),  "ConstrXeqYmodC::LocalArcCons: Wrong getVarFired");
...
void
ConstrXneqY::LocalArcCons (QueueItem& Qitem)
{
        if (VarX == Qitem.getVarFired()) {
                if ( VarX->isBound()   &&   VarY->contains( VarX->value() ) )
                        VarY->removeSingle( VarX->value(), this );
        } else {
                assert_exc(VarY == Qitem.getVarFired(),  "ConstrXneqY::LocalArcCons: Wrong getVarFired");
                if ( VarY->isBound()   &&   VarX->contains( VarY->value() ) )
                        VarX->removeSingle( VarY->value(), this );
        }
}
...
void
ConstrXeqAbsY::LocalArcCons (QueueItem& Qitem)
{
        if (VarX == Qitem.getVarFired()) {
                VarY->removeRange(NsMINUS_INF, -(VarX->max() + 1), this);
                VarY->removeRange(+(VarX->max() + 1), NsPLUS_INF, this);
                ...
                assert_exc(VarY == Qitem.getVarFired(),  "ConstrXeqAbsY::LocalArcCons: Wrong getVarFired");
                VarX->removeRange(NsMINUS_INF, max(labs( max(VarY->min(),static_cast<NsInt>(0)) ), labs( min(static_cast<NsInt>(0),VarY->max()))) - 1, this);
                VarX->removeRange(max(labs(VarY->min()),labs(VarY->max())) + 1, NsPLUS_INF, this);
        }
}
...
bool
naxos::IntersectionEmpty (const NsIntVar *VarY, const NsIntVar *VarZ)
{
...
//
//    	void
//    MetaXeqYeqneqZLocalArcCons (QueueItem& Qitem, NsIntVar *VarX, NsIntVar *VarY, NsIntVar *VarZ,
//    		const NsConstraint *constraint, const bool neg)
//    {
//    }
//}  // namespace
...
void
ConstrMetaXeqYeqZ::ArcCons (void)
{
        //MetaXeqYeqneqZArcCons(VarX, VarY, VarZ, this, false);
        MakeBoolean(VarX, this);
        if (VarX->contains(neg)  &&  VarY->max()==VarZ->min() && VarY->min()==VarZ->max())
        ...
        if (VarX->isBound()) {
                if (VarX->value() == neg) {
                        ...
                        assert_exc(VarX->value() == !neg,  "MetaXeqYeqneqZArcCons: Wrong `VarX->value()'");
                        NsIntVar::const_iterator  v;
                        for (v = VarY->begin();  v != VarY->end();  ++v) {
                                if ( !VarZ->contains( *v ) )
                                        VarY->removeSingle( *v, this );
...
void
ConstrMetaXeqYeqZ::LocalArcCons (QueueItem& Qitem)
{
        ...
        if (VarX == Qitem.getVarFired()) {
                if ( VarX->isBound() ) {
                        if (VarX->value() == neg) {
                                ...
                                assert_exc(VarX->value() == !neg,
                                           "ConstrMetaXeqYeqZ::LocalArcCons: Wrong `VarX->value()'");
                                NsIntVar::const_iterator  v;
                                for (v = VarY->begin();  v != VarY->end();  ++v) {
                                        if ( !VarZ->contains( *v ) )
                                                VarY->removeSingle( *v, this );
                ...
                if (VarY == Qitem.getVarFired()) {
                        ...
                        assert_exc(VarZ == Qitem.getVarFired(),
                                   "ConstrMetaXeqYeqZ::LocalArcCons: Wrong getVarFired");
                        if ( VarX->isBound() ) {
                                if (VarX->value() == neg) {
                                        if ( VarZ->isBound()   &&   VarY->contains( VarZ->value() ) )
                                                VarY->removeSingle( VarZ->value(), this );
...
ConstrXeqYandZ::ArcCons (void)
{
        ...
        if ( VarX->isBound() ) {
                if (VarX->value() == neg) {
                        ...
                        assert_exc(VarX->value() == !neg,
                                   "ConstrXeqYandZ::LocalArcCons: Wrong `VarX->value()'");
                        if ( VarY->contains(0) )
...
void
ConstrXeqYandZ::LocalArcCons (QueueItem& Qitem)
{
        if (VarX == Qitem.getVarFired()) {
                if ( VarX->isBound() ) {
                        if (VarX->value() == neg) {
                ...
                //assert_exc(VarY == Qitem.getVarFired(),  "ConstrXeqYandZ::LocalArcCons: Wrong getVarFired");
                if (VarX->contains(neg)  &&  (VarY->min() * VarZ->min()))
                        VarX->removeSingle( neg, this );
                if (VarX->contains(!neg)  &&  !(VarY->max() * VarZ->max()))
                        VarX->removeSingle( !neg, this );
...
ConstrXeqYorZ::ArcCons (void)
{
        ...
        if ( VarX->isBound() ) {
                if (VarX->value() == neg) {
                        ...
                        assert_exc(VarX->value() == !neg,
                                   "ConstrXeqYorZ::LocalArcCons: Wrong `VarX->value()'");
                        //VOID
                }
        }
}
...
void
ConstrXeqYorZ::LocalArcCons (QueueItem& Qitem)
{
        if (VarX == Qitem.getVarFired()) {
                if ( VarX->isBound() ) {
                        if (VarX->value() == neg) {
                ...
                //assert_exc(VarY == Qitem.getVarFired(),  "ConstrXeqYorZ::LocalArcCons: Wrong getVarFired");
                if (VarX->contains(neg)  &&  (VarY->min() + VarZ->min() != 0))
                        VarX->removeSingle( neg, this );
                if (VarX->contains(!neg)  &&  (VarY->max() + VarZ->max() == 0))
                        VarX->removeSingle( !neg, this );
...
void
ConstrXorY::LocalArcCons (QueueItem& Qitem)
{
        //NsInt  SupportVal = Qitem.getW();
        if (VarX == Qitem.getVarFired()) {
                if (VarX->isBound()  &&  VarX->value()==neg  &&  VarY->contains(neg))
                ...
                assert_exc(VarY == Qitem.getVarFired(),  "ConstrXorY::LocalArcCons: Wrong getVarFired");
                if (VarY->isBound()  &&  VarY->value()==neg  &&  VarX->contains(neg))
                        VarX->removeSingle( neg, this );
        }
}
...
void
naxos::array_min_minmax (const NsIntVarArray *VarArr, NsInt& min, NsInt& minmax)
{
...
void
naxos::array_maxmin_max (const NsIntVarArray *VarArr, NsInt& maxmin, NsInt& max)
{
        NsIntVarArray::const_iterator  V = VarArr->begin();
        maxmin = V->min();
        max = V->max();
...
void
naxos::array_sum_min_max (const NsIntVarArray *VarArr, const NsIndex start, const NsIndex length,
...
{
        assert_exc(VarArr->size() >= 1,
                   "ConstrXeqMin::ConstrXeqMin: Condition required:  VarArr.size() >= 1");
        NsProblemManager&  pm = VarX->manager();
        for (NsIntVarArray::iterator  V = VarArr->begin();   V != VarArr->end();   ++V) {
                assert_exc( &pm == &V->manager(),  "ConstrXeqMin::ConstrXeqMin: All the variables of a constraint must belong to the same NsProblemManager");
        }
}
...
{
        assert_exc(VarArr->size() >= 1,
                   "ConstrXeqMax::ConstrXeqMax: Condition required:  VarArr.size() >= 1");
        NsProblemManager&  pm = VarX->manager();
        for (NsIntVarArray::iterator  V = VarArr->begin();   V != VarArr->end();   ++V) {
                assert_exc( &pm == &V->manager(),  "ConstrXeqMax::ConstrXeqMax: All the variables of a constraint must belong to the same NsProblemManager");
        }
}

void
ConstrXeqMin::ArcCons (void)
{
        NsInt  min, minmax;
        ...
        for ( ;  v != VarX->end();  ++v) {
                //for (NsInt  val = VarX->min() + 1;   val < VarX->max();   ++val)   {
                //if ( VarX->contains( val ) )  {
...
//namespace  {
//		void
//	MinMaxLocalArcCons (QueueItem& Qitem, NsIntVar *VarX, NsIntVarArray *VarArr, const NsConstraint *constraint)
//	{
//		NsIntVarArray::iterator  V;
//
//		if (VarX == Qitem.getVarFired())  {
...
void
ConstrXeqMin::LocalArcCons (QueueItem& Qitem)
{
...
void
ConstrXeqMax::LocalArcCons (QueueItem& Qitem)
{
        //MinMaxLocalArcCons(Qitem, VarX, VarArr, this);
        NsIntVarArray::iterator  V;
        if (VarX == Qitem.getVarFired()) {
...
{
        assert_exc(VarArr->size() >= 1,
                   "ConstrXeqSum::ConstrXeqSum: Condition required:  VarArr.size() >= 1");
        NsProblemManager&  pm = VarX->manager();
        for (NsIntVarArray::iterator  V = VarArr->begin();   V != VarArr->end();   ++V) {
                assert_exc( &pm == &V->manager(),  "ConstrXeqSum::ConstrXeqSum: All the variables of a constraint must belong to the same NsProblemManager");
        }
}
...
// bounds-consistency only
void
ConstrXeqSum::LocalArcCons (QueueItem& Qitem)
{
...
// bounds-consistency only
void
ConstrXeqYplusZ::LocalArcCons (QueueItem& Qitem)
{
        if (Qitem.getVarFired()->min() < Qitem.getW()  &&  Qitem.getW() < Qitem.getVarFired()->max())
                return;					 // bounds-consistency does not care
        ArcCons();
...
void
naxos::product_min_max (const NsIntVar *VarY, const NsIntVar *VarZ, NsInt& min, NsInt& max)
{
        NsInt  prod;
...
// bounds-consistency only
void
ConstrXeqYtimesZ::LocalArcCons (QueueItem& Qitem)
{
        if (Qitem.getVarFired()->min() < Qitem.getW()  &&  Qitem.getW() < Qitem.getVarFired()->max())
                return;					 // bounds-consistency does not care
        ArcCons();
...
ConstrAllDiff::ConstrAllDiff (NsIntVarArray *VarArr_init)
        : VarArr(VarArr_init)
{
        assert_exc(VarArr->size() >= 2,
                   "ConstrAllDiff::ConstrAllDiff: Condition required:  VarArr.size() >= 2");
        NsIntVarArray::iterator  X = VarArr->begin();
        NsProblemManager&  pm = X->manager();
        ++X;
        for ( ;   X != VarArr->end();   ++X) {
                assert_exc( &pm == &X->manager(),  "ConstrAllDiff::ConstrAllDiff: All the variables of a constraint must belong to the same NsProblemManager");
        }
}

void
ConstrAllDiff::ArcCons (void)
{
...
void
ConstrAllDiff::LocalArcCons (QueueItem& Qitem)
{
...
bool
ConstrAllDiffStrong::groupedNsIntVar::removeDomain (const IntDomain& V, const NsConstraint *constraint)
{
        if ( Var.max()  <=  V.max() ) {
...
ConstrAllDiffStrong::ConstrAllDiffStrong (NsIntVarArray *VarArr_init, unsigned long Cap)
        : Capacity(Cap)
{
        assert_exc( VarArr_init->size() > Capacity,
                    "ConstrAllDiffStrong::ConstrAllDiffStrong: Condition required:  VarArr.size() >= 2");
        assert_exc( Capacity > 0,
                    "ConstrAllDiffStrong::ConstrAllDiffStrong: Condition required:  Capacity > 0");
        NsIntVarArray::iterator  V = VarArr_init->begin();
        NsProblemManager&  pm = V->manager();
        for ( ;   V != VarArr_init->end();   ++V) {
                assert_exc( &pm == &V->manager(),  "ConstrAllDiffStrong::ConstrAllDiffStrong: All the variables of a constraint must belong to the same NsProblemManager");
                VarArr.push_back( groupedNsIntVar(*V) );
                VarPointerGroup.insert( make_pair( (pointer_t)&*V  ,  &VarArr.back() ) );
        }
        //for (X = VarArr->begin();   X != VarArr->end();   ++X)
...
{
        NsIntVar&  VarX = (*static_cast<ConstrAllDiffStrong::groupedNsIntVar *const *>( X ))->Var;
        NsIntVar&  VarY = (*static_cast<ConstrAllDiffStrong::groupedNsIntVar *const *>( Y ))->Var;
...
void
AllDiffBoundsConsistency (NsDeque<ConstrAllDiffStrong::groupedNsIntVar>& VarArr,
                          ...
                          const NsConstraint *constraint)
{
        ...
        assert_exc( VarArrSortedList.size() != 0,  "AllDiffBoundsConsistency: Empty `group' of variables" );
        ...
        qsort(VarArrSorted, VarArrSortedList.size(), sizeof(ConstrAllDiffStrong::groupedNsIntVar *),
        ...
        for (NsIndex max_i = 0;   max_i < Max.size();   ++max_i) {
                ...
                IntDomain  *vUnion = new IntDomain(VarLeMax[0]->Var.min(), VarLeMax[0]->Var.max());
                NsIntVar::const_gap_iterator  gapVal;
                for (gapVal = VarLeMax[0]->Var.gap_begin();  gapVal != VarLeMax[0]->Var.gap_end();  ++gapVal) {
                        vUnion->reset(*gapVal);
                        //assert_exc( vUnion->removeSingle(*gapVal, constraint),
                        //		"ConstrAllDiffStrong::ArcCons: `vUnion->removeSingle(*gapVal)' failed" );
                }
                                ...
                                for (gapVal = vUnionPrevious.gap_begin();
                                     ...
                                     ++gapVal) {
                                        if ( ! VarLeMax[j]->Var.contains( *gapVal ) ) {
                                                vUnion->reset(*gapVal);
                                                //assert_exc( vUnion->removeSingle(*gapVal,constraint),  "ConstrAllDiffStrong::ArcCons: `vUnion->removeSingle(*gapVal)' failed" );
                                        }
                                }
...
// bounds-consistency only
void
ConstrAllDiffStrong::ArcCons (void)
{
...
// bounds-consistency only
void
ConstrAllDiffStrong::LocalArcCons (QueueItem& Qitem)
{
        if (Qitem.getVarFired()->min() < Qitem.getW()  &&  Qitem.getW() < Qitem.getVarFired()->max())
                return;					 // bounds-consistency does not care
        VarPointerGroup_t::const_iterator  cit = VarPointerGroup.find( (pointer_t)Qitem.getVarFired() );
          ...
          Values(Values_init), Occurrences(Occurrences_init)
{
        ...
        ++X;
        for ( ;   X != VarArr->end();   ++X) {
                assert_exc( pm == &X->manager(),  "Ns_ConstrCount::Ns_ConstrCount: All the variables of a constraint must belong to the same NsProblemManager");
        }
        ...
        for (NsDeque<NsInt>::const_iterator v=Values.begin();
             ...
             ++v, ++i) {
                ...
                //assert_exc( *v >= 0 , "Ns_ConstrCount::Ns_ConstrCount: Negative value in `Values'");
                //vCount.push_back( NsIntVar(*pm, 0, Occurrences[i]) );
...
void
countArcCons (const NsIntVar& Var,
...
void
Ns_ConstrCount::LocalArcCons (QueueItem& Qitem)
{
...
//
//			for ( ;   X != VarArr->end();   ++X)   {
//				assert_exc(X->getID() == previous + 1,  "MakeIDs: Non-continuous ids in `VarArr'");
//				previous = X->getID();
//			}
//
//		} else  {
...
ConstrInverse::ConstrInverse (NsIntVarArray *VarArrInv_init, NsIntVarArray *VarArr_init)
        : VarArrInv(VarArrInv_init),  VarArr(VarArr_init), VArrInv(*VarArrInv), VArr(*VarArr)
{
        assert_exc(VarArrInv->size() >= 1  &&  VarArr->size() >= 1,
                   "ConstrInverse::ConstrInverse: Condition required:  Both arrays must have some elements");
        NsIntVarArray::iterator  X = VarArr->begin();
        NsProblemManager&  pm = X->manager();
        ++X;
        for ( ;   X != VarArr->end();   ++X) {
                assert_exc( &pm == &X->manager(),  "ConstrInverse::ConstrInverse: All the variables of a constraint must belong to the same NsProblemManager");
        }
        ...
        for (X=VarArr->begin(), i=0;   X != VarArr->end();   ++X, ++i) {
                ...
                VarArrayIndex.insert( make_pair( (pointer_t)&*X  ,  ArrayIndex_t( false , i ) ) );
        }
        for (X=VarArrInv->begin(), i=0;   X != VarArrInv->end();   ++X, ++i) {
                assert_exc( VarArrayIndex.count( (pointer_t)&*X ) == 0,
                            "ConstrInverse::ConstrInverse: Duplicate NsIntVar");
                VarArrayIndex.insert( make_pair( (pointer_t)&*X  ,  ArrayIndex_t( true , i ) ) );
        }
        //MakeIDs(VarArr);
        //MakeIDs(VarArrInv);
}
        ...
        for (i = 0;   i < VArrInv.size();   ++i) {
                for (val = VArrInv[i].begin();  val != VArrInv[i].end();   ++val) {
                        if (*val != -1) {
                                ...
                                else if (VArr[*val].isBound()  &&  VArrInv[i].contains(-1))
...
void
ConstrInverse::LocalArcCons (QueueItem& Qitem)
{
        ...
        if  (VarFiredW < 0)
                return;
        VarArrayIndex_t::const_iterator  cit = VarArrayIndex.find( (pointer_t)Qitem.getVarFired() );
        NsIndex  VarFiredId = cit->second.index;
        //if (VArr[0].getID() <= VarFiredId   &&   VarFiredId < VArr[0].getID() + VArr[0].size())  {
        if ( cit->second.InInverse ) {
                //assert_exc(VArrInv[0].getID() <= VarFiredId  &&  VarFiredId < VArrInv[0].getID() + VArrInv[0].size(),
                //		"ConstrInverse::LocalArcCons: Wrong `VarFiredId'");
                //VarFiredId -= VArrInv[0].getID();
                ...
                if ( Qitem.getVarFired()->isBound() ) {
                        ...
                        assert_exc(0<=val && static_cast<unsigned>(val) < VArrInv.size(),
...
void
Ns_ConstrElement::LocalArcCons (QueueItem& Qitem)
{
