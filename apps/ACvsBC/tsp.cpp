        ...
        try {
                ...
                while ( getline(file,input) ) {
                        ...
                        if ( tok == "costs"  ||  tok == "       " ) {
                                ...
                                while ( ! (tok = tokens.next()).empty()  &&  tok != ")." )
                                        edge_weight.back().push_back(atof(tok.c_str())*precision + 0.5);
                        } else {
                ...
                for (i=0;  i < edge_weight.size();  ++i) {
                        ...
                        for (j=0;  j < i;  ++j)
                                edge_weight[i].push_front(edge_weight[i-j-1][i]);
                }
                ...
                //  We impose the `element' constraint using the edge_weight_onedim.
                NsIntVarArray  vWeights;
                for (i=0;  i < N-1;  ++i)
                        vWeights.push_back( edge_weight_onedim[ R[i]*N + R[i+1] ] );
