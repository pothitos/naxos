...
{
	if  ( static_cast<const struct VarHeur_t*>( X ) -> heur  >  static_cast<const struct VarHeur_t*>( Y ) -> heur )
		return  -1;
	else if  ( static_cast<const struct VarHeur_t*>( X ) -> heur  <  static_cast<const struct VarHeur_t*>( Y ) -> heur )
		return  +1;
	else
		return   0;
}
...
main (int argc, char *argv[])
{
	...
	//  We double-state the problem.  In the second statement we have the  //
	//   extension "_ls" to the variables and problem manager (`pm_ls'),   //
	//   because we will use them to implement a Local Search technique.   //

	NsProblemManager  pm;
	if ( saveSearchTree )
		pm.searchToGraphFile( (string(argv[2]) + ".dot").c_str() );

	NsProblemManager  pm_ls;
	if ( saveSearchTree  &&  pr.timeLimitDirectMethodRound < pr.timeLimit )
		pm_ls.searchToGraphFile( (string(argv[2]) + ".indirect.dot").c_str() );
	...
	for (k=0,i=0;  i < pr.ncourses;  ++i)   {
		...
		courseInfo[i].lastLectIndex  = k + pr.course[i].lectures - 1;
		...
		for (j=0;  j < pr.course[i].lectures;  ++j,++k)   {
			...
			vLectPeriod.push_back( NsIntVar(pm, 0, pr.days*pr.hours-1) );
			vLectPeriod_ls.push_back( NsIntVar(pm_ls, 0, pr.days*pr.hours-1) );

			/* Availability hard constraint */

			for (l=0;  l < pr.course[i].nunavail;  ++l)   {

				vLectPeriod[k].remove( pr.course[i].unavail[l] );
				vLectPeriod_ls[k].remove( pr.course[i].unavail[l] );
			}

			/* Lecture distinct period assignment hard constraint */

			if ( j > 0 )   {

				pm.add( vLectPeriod[k-1] < vLectPeriod[k] );
				pm_ls.add( vLectPeriod_ls[k-1] < vLectPeriod_ls[k] );
			}

			/* Room Occupancy hard constraint */

			vLectRoom.push_back( NsIntVar(pm, 0, pr.nrooms-1) );
			vLectRoom_ls.push_back( NsIntVar(pm_ls, 0, pr.nrooms-1) );

			vRooms.push_back( NsYplusCZspecial(vLectPeriod[k], pr.days * pr.hours, vLectRoom[k]) );
			vRooms_ls.push_back( NsYplusCZspecial(vLectPeriod_ls[k], pr.days * pr.hours, vLectRoom_ls[k]) );
			//vRooms.push_back( vLectRoom[k] * pr.days * pr.hours + vLectPeriod[k] );
			//vRooms_ls.push_back( vLectRoom_ls[k] * pr.days * pr.hours + vLectPeriod_ls[k] );

			/* Conflict (of Teachers) hard constraint */

			vProfessor[ pr.course[i].professor ].push_back( vLectPeriod[k] );
			vProfessor_ls[ pr.course[i].professor ].push_back( vLectPeriod_ls[k] );
		}

		for (j=0;  j <= i;  ++j)   {
			// In purpose, we used greater or *equal* in order to connect each course with itself

			if ( pr.course[j].professor == pr.course[i].professor )
				courseInfo[i].isConnectedToCourse[j] = courseInfo[j].isConnectedToCourse[i] = true;
		}
	}
	...
	for (i=0;  i < pr.ngroups;  ++i)   {
		...
		for (j=0;  j < pr.group[i].ncourses;  ++j)   {
			...
			courseInfo[ pr.group[i].course[j] ].group.push_back( i );

			for ( k =  courseInfo[ pr.group[i].course[j] ].firstLectIndex;
			      k <= courseInfo[  pr.group[i].course[j] ].lastLectIndex;
			      ++k )
			{
				vGroup[i].push_back( vLectPeriod[k] );
				vGroup_ls[i].push_back( vLectPeriod_ls[k] );
			}

			for (k=0;  k < j;  ++k)   {

				l = pr.group[i].course[k];
				courseInfo[l].isConnectedToCourse[j] = courseInfo[j].isConnectedToCourse[l] = true;
			}
		}
	...
	NsDeque<NsIntVarArray>  vCLectRoom(pr.ncourses), vCRoomLect(pr.ncourses),
		vLectDay(pr.ncourses), vDayLect(pr.ncourses), vIsBusyDay(pr.ncourses),
	...
	NsDeque<NsIntVarArray>  vCLectRoom_ls(pr.ncourses), vCRoomLect_ls(pr.ncourses),
		vLectDay_ls(pr.ncourses), vDayLect_ls(pr.ncourses), vIsBusyDay_ls(pr.ncourses),
		vObjectiveCapacity_ls(vLectPeriod.size());
	NsIntVarArray  vObjectiveCapacityTerms_ls, vBusyRooms_ls, vBusyDays_ls,
		       vObjectiveMinWorkingDays_ls, vObjectiveTerms_ls;
	...
	for (i=0;  i < pr.ncourses;  ++i)   {
		...
		for (j = courseInfo[i].firstLectIndex;   j <= courseInfo[i].lastLectIndex;   ++j)   {
			...
			for (k=0;  k < pr.nrooms;  ++k)   {
				...
				if ( pr.course[i].students > pr.room[k].capacity )   {

					vObjectiveCapacity[j].push_back(
							( pr.course[i].students - pr.room[k].capacity ) *
							( vLectRoom[j] == k ) );
					vObjectiveCapacity_ls[j].push_back(
							( pr.course[i].students - pr.room[k].capacity ) *
							( vLectRoom_ls[j] == k ) );
				}
			}
			...
			if ( vObjectiveCapacity[j].size()  >  0 )   {
				...
				vObjectiveCapacityTerms.push_back( NsMax( vObjectiveCapacity[j] ) );
				vObjectiveCapacityTerms_ls.push_back( NsMax( vObjectiveCapacity_ls[j] ) );
			}

			/* We could use `NsSum' instead of `NsMax'.  We did use the latter,
			 * because it provides a better upper bound of the objective function. */
		}
		...
		for (j=0;  static_cast<unsigned>(j) < vCRoomLect[i].size();  ++j)   {

			vBusyRooms.push_back( vCRoomLect[i][j] >= 0 );
			vBusyRooms_ls.push_back( vCRoomLect_ls[i][j] >= 0 );
		}
		...
		for (j = courseInfo[i].firstLectIndex;  j <= courseInfo[i].lastLectIndex;  ++j)   {
			...
			vLectDay[i].push_back( vLectPeriod[j] / pr.hours );
			vLectDay_ls[i].push_back( vLectPeriod_ls[j] / pr.hours );
		}
		...
		for (d=0;  d < pr.days;  ++d)   {
			...
			vIsBusyDay_ls[i].push_back( vDayLect_ls[i][d] >= 0 );
		}
	...
	vObjectiveTerms_ls.push_back( 5 * NsSum( vObjectiveMinWorkingDays_ls ) );

	/* Curriculum Compactness soft constraint */

	NsDeque<NsIntVarArray>  vPeriodLect(pr.ngroups), vIsolatedLect(pr.ngroups);
	NsDeque<NsIntVarArray>  vPeriodLect_ls(pr.ngroups), vIsolatedLect_ls(pr.ngroups);
	NsIntVarArray  vIsolatedLectSums;
	NsIntVarArray  vIsolatedLectSums_ls;

	for (i=0;  i < pr.ngroups;  ++i)   {

		vPeriodLect[i] = NsInverse( vGroup[i],  pr.days * pr.hours - 1 );
		vPeriodLect_ls[i] = NsInverse( vGroup_ls[i],  pr.days * pr.hours - 1 );
		...
		for (d=0;  d < pr.days;  ++d)   {
			...
			if ( pr.hours == 1 )   {
				...
				vIsolatedLect[i].push_back( vPeriodLect[i][ d*pr.hours + h ] >= 0 );
				vIsolatedLect_ls[i].push_back( vPeriodLect_ls[i][ d*pr.hours + h ] >= 0 );
			}

			if ( pr.hours > 1 )   {

				vIsolatedLect[i].push_back( vPeriodLect[i][ d*pr.hours + h ] >= 0
					      	&&  vPeriodLect[i][ d*pr.hours + h + 1 ] < 0 );
				vIsolatedLect_ls[i].push_back( vPeriodLect_ls[i][ d*pr.hours + h ] >= 0
					      	&&  vPeriodLect_ls[i][ d*pr.hours + h + 1 ] < 0 );
			}
		...
		vIsolatedLectSums_ls.push_back( NsSum( vIsolatedLect_ls[i] ) );
	}

	vObjectiveTerms.push_back( 2 * NsSum( vIsolatedLectSums ) );
	vObjectiveTerms_ls.push_back( 2 * NsSum( vIsolatedLectSums_ls ) );

	vObjectiveTerms.push_back( NsSum( vBusyRooms ) - pr.ncourses );	// for the `Room Stability soft constraint'
	vObjectiveTerms_ls.push_back( NsSum( vBusyRooms_ls ) - pr.ncourses );
	...
	for (i=0;  i < pr.ncourses;  ++i)   {
		...
		double  current2ndVarHeurCriterion = unboundVarsConnecedTo(i, courseInfo, vLectPeriod, pr);
		double  current3rdVarHeurCriterion = varHeurObjectiveCriterion(i, courseInfo, pr, vBusyDays);

		if ( current2ndVarHeurCriterion > pr.max2ndVarHeurCriterion )
			pr.max2ndVarHeurCriterion = current2ndVarHeurCriterion;

		if ( current3rdVarHeurCriterion > pr.max3rdVarHeurCriterion )
			pr.max3rdVarHeurCriterion = current3rdVarHeurCriterion;
	}
	...
	//  When a static variable ordering heuristic is used by some methods, we take for     //
	//   granted that the assignments are made according to the order that each variable   //
	//   appears in the variables array (i.e. the first variable is chosen).  So we sort   //
	//   this array according to the values that the variable ordering heuristic function  //
	//   initially gives; an array `vLectPeriodSortedByHeur' is produced.                  //

	struct VarHeur_t   *allVarsLectPeriod  =  new struct VarHeur_t[vLectPeriod.size()];

	for (k=0,i=0;  i < pr.ncourses;  ++i)   {

		for (j = courseInfo[i].firstLectIndex;   j <= courseInfo[i].lastLectIndex;   ++j,++k)   {

			allVarsLectPeriod[k].index  = k;
			allVarsLectPeriod[k].heur = 
					heurEstimationForVariable(k, i, courseInfo, vLectPeriod, pr, vBusyDays);
	...
	//sort(allVarsLectPeriod.begin(), allVarsLectPeriod.end(), less_function_VarHeur());

	NsIntVarArray  vLectPeriodSortedByHeur, vLectRoomSortedByHeur;
	NsIntVarArray  vLectPeriodSortedByHeur_ls, vLectRoomSortedByHeur_ls;
	
	for (i=0;  static_cast<unsigned>(i) < vLectPeriod.size();  ++i)   {

		vLectPeriodSortedByHeur.push_back( vLectPeriod[allVarsLectPeriod[i].index] );
		vLectPeriodSortedByHeur_ls.push_back( vLectPeriod_ls[allVarsLectPeriod[i].index] );
	...
	NsDeque<unsigned>    lanAssigns;	// used only for the LAN search method
	NsDeque<unsigned>    lanAssigns_ls;


	VarHeurTimetabling  varHeur(((pr.searchMethod==DBDS)? vLectRoomSortedByHeur : vLectRoom),
					pr, lectInfo, courseInfo, lanAssigns, vRooms, vBusyDays);
	VarHeurTimetabling  varHeur_ls(((pr.searchMethod_ls==DBDS)? vLectRoomSortedByHeur_ls : vLectRoom_ls),
					pr, lectInfo, courseInfo, lanAssigns_ls, vRooms_ls, vBusyDays_ls);

	ValHeurTimetabling  valHeur(vLectPeriod, vLectRoom, pr, lectInfo, courseInfo, vRooms,
					vLectDay, vIsBusyDay, vBusyDays, vGroup, vIsolatedLect);
	ValHeurTimetabling  valHeur_ls(vLectPeriod_ls, vLectRoom_ls, pr, lectInfo, courseInfo, vRooms_ls,
					vLectDay_ls, vIsBusyDay_ls, vBusyDays_ls, vGroup_ls, vIsolatedLect_ls);

	SplitValHeurTimetabling  splitValHeur(vLectPeriod, vLectRoom, pr, lectInfo, courseInfo, vRooms);
	SplitValHeurTimetabling  splitValHeur_ls(vLectPeriod_ls, vLectRoom_ls, pr, lectInfo, courseInfo, vRooms_ls);
	...
	switch ( pr.searchMethod )   {
		...
		case  PoPS :
			...
			if ( ! isPoPS )
				pm.addGoal( new AmPopsLabeling(vLectPeriod, pr.searchMethodParameter[0]/100.0, &varHeur, &valHeur) );	// PopsSample
			break;
	...
	if ( isPoPS )   {
	  ...
	  while ( ! searchEnded )   {
		...
		while ( pm.nextSolution() != false )   {
			...
			timeBest  =  DiffTime(time(0), timeStarted);
		...
		searchEnded =
		( DiffTime(time(0),timeStarted) >= pr.timeLimit )  ||
	...
	while ( ! isPoPS )   {
		...
		if ( pr.timeLimitIndirectMethodRound != 0 )   {
			...
			pr.timeLimitIndirectMethodRound =
				min( pr.timeLimitIndirectMethodRound , static_cast<int>(timeLeft) );
		...
		if ( pr.searchMethod_ls  !=  NONE )
			pm_ls.addGoal( new goalLsWeek(vLectPeriod_ls, vLectPeriodSortedByHeur_ls, vLectRoom_ls, pr,
						varHeur_ls, valHeur_ls, lanAssigns_ls, (currentDay++)%pr.days) );
	...
	NsGoal*
metaSearchMethodGoal (struct itcProblem_t& pr, NsIntVarArray& vLectPeriod, NsIntVarArray& vLectPeriodSortedByHeur,
		VariableHeuristic& varHeur, ValueHeuristic& valHeur, NsDeque<unsigned>& lanAssigns)
{
