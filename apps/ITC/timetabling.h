...
enum  itcSearchMethod_t  {NONE, DFS, LDS, CREDIT, DBS, LAN, DBDS, IBROAD,
	BBS, RDFS, GNS, FNS, ISAMP, ONESAMP, DOMAIN_SPLITTING, PoPS};
enum  itcHeurType_t  {NORMAL=1, STATIC, RANDOM, FAIR /* for domain-splitting */ };
...
void  writeSolution (const struct itcProblem_t& pr, naxos::NsIntVarArray& vLectPeriod, naxos::NsIntVarArray& vLectRoom,
			const char *solutionFileName);

naxos::NsGoal*  metaSearchMethodGoal (struct itcProblem_t& pr,
		naxos::NsIntVarArray& vLectPeriod, naxos::NsIntVarArray& vLectPeriodSortedByHeur, 
		naxos::VariableHeuristic& varHeur, naxos::ValueHeuristic& valHeur, naxos::NsDeque<unsigned>& lanAssigns);
