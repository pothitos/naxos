...
class VarHeurTimetabling : public naxos::VariableHeuristic {
    ...
    public:
        VarHeurTimetabling (naxos::NsIntVarArray& vLectRoom_init,
                ...
                : vLectRoom(vLectRoom_init), pr(pr_init), lectInfo(lectInfo_init), courseInfo(courseInfo_init),
                  lanAssigns(lanAssigns_init), vRooms(vRooms_init), vBusyDays(vBusyDays_init),
...
class ValHeurTimetabling : public naxos::ValueHeuristic {
    ...
    public:
        ValHeurTimetabling (const naxos::NsIntVarArray& vLectPeriod_init,
                ...
                : vLectPeriod(vLectPeriod_init), vLectRoom(vLectRoom_init), pr(pr_init),
                  lectInfo(lectInfo_init), courseInfo(courseInfo_init),
                  vRooms(vRooms_init), vLectDay(vLectDay_init),
                  vIsBusyDay(vIsBusyDay_init), vBusyDays(vBusyDays_init),
                  vGroup(vGroup_init), vIsolatedLect(vIsolatedLect_init)
        {    }
...
class SplitValHeurTimetabling : public naxos::SplitValHeuristic {
    ...
    public:
        SplitValHeurTimetabling (const naxos::NsIntVarArray& vLectPeriod_init,
                                 const naxos::NsIntVarArray& vLectRoom_init,
                                 const struct itcProblem_t& pr_init,
                                 const naxos::NsDeque<struct itcLectureInfo_t>& lectInfo_init,
                                 const naxos::NsDeque<struct itcCourseInfo_t>& courseInfo_init,
...
double  heurEstimationForVariable (const int index, const int courseIndex,
                                   const naxos::NsDeque<struct itcCourseInfo_t>& courseInfo, const naxos::NsIntVarArray& vLectPeriod,
                                   const struct itcProblem_t& pr, const naxos::NsIntVarArray& vBusyDays);

int  unboundVarsConnecedTo (const int courseIndex, const naxos::NsDeque<struct itcCourseInfo_t>& courseInfo,
                            const naxos::NsIntVarArray& vLectPeriod, const struct itcProblem_t& pr);

int  varHeurObjectiveCriterion (const int courseIndex, const naxos::NsDeque<struct itcCourseInfo_t>& courseInfo,
                                const struct itcProblem_t& pr, const naxos::NsIntVarArray& vBusyDays);
#endif							 // HEURISTICS_H
