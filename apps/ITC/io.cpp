...
struct less_function_RoomRank : public binary_function<struct RoomRank_t, struct RoomRank_t, bool>  {
	...
	void
createInstance (struct itcProblem_t& pr, const int argc, char *argv[])
/* Reading input file, i.e. an instance of the timetabling problem */
{
	...
	if ( argc < 3 )   {
		cerr << "Usage:  " << argv[0] << " <input_file> <results_file> [<random_seed>] [-options <options_file>]\n"
		     << "          The above command solves the timetabling problem described by\n"
		     << "          <input_file> and writes the solution to the <results_file>.\n"
		     //<< "          A <results_file>.plt file is also produced and can be used by the\n"
		     //<< "          program \"gnuplot\" to visualize the progress of the search methods.\n"
		     << "          To override the default search options, use the \"-options\" parameter.\n"
		     << "          (See the provided \"optionsFile\" for an example.)  If a random search\n"
		     << "          method is selected, a <random_seed> number can be defined.\n"
		     << "\n"
		     << "        " << argv[0] << " <input_file> <results_file> -fct\n"
		     << "          The above command takes an existing <results_file> corresponding\n"
		     << "          to <input_file> and produces a <results_file>.fct which can be used\n"
		     << "          to visualize the results using the external program \"SKOPtiko.\"\n";
	...
	instanceFile >> str;
	assert_that( str == "Periods_per_day:",  "Expected token \"Periods_per_day\"" );
	...
	for (i=0;  i < pr.ncourses;  ++i)   {
		...
		instanceFile >> str;
		assert_that( (pr.course[i].name = strdup(str.c_str())) != 0,  "Out of memory" );
		...
		if ( j == pr.nprofessors )   {
			...
			++pr.nprofessors;
			assert_that( (pr.professorName[j] = strdup(str.c_str())) != 0,  "Out of memory" );
		}
		/* Found index `j' */
		pr.course[i].professor = j;

		instanceFile >> pr.course[i].lectures >> pr.course[i].minWorkingDays >> pr.course[i].students;
		assert_that( pr.course[i].lectures > 0  &&  pr.course[i].minWorkingDays > 0
				&&  pr.course[i].students >= 0,  "Wrong course attribute" );

		pr.course[i].nunavail = 0;
		if ( pr.nConstraints > 0 )   {
			pr.course[i].unavail = new int[ pr.nConstraints ];

			/* The number of a course unavailabilities cannot be greater
			 * than the aggregate number of unavailabilities (`pr.nConstraints'). */
		}
	...
	for (i=0;  i < pr.nrooms;  ++i)   {
		...
		instanceFile >> str;
		assert_that( (pr.room[i].name = strdup(str.c_str())) != 0,  "Out of memory" );
	...
	for (i=0;  i < pr.ngroups;  ++i)   {
		...
		instanceFile >> str;
		assert_that( (pr.group[i].name = strdup(str.c_str())) != 0,  "Out of memory" );

		instanceFile >> pr.group[i].ncourses;
		assert_that( pr.group[i].ncourses > 0,  "Wrong group courses number" );
		...
		for (j=0;  j < pr.group[i].ncourses;  ++j)   {
			...
			assert_that( k < pr.ncourses,  "Invalid course for a group" );
			/* Found index `k' */
	...
	instanceFile >> str;
	assert_that( str == "UNAVAILABILITY_CONSTRAINTS:",  "Expected token \"UNAVAILABILITY_CONSTRAINTS\"" );
	...
	for (i=0;  i < pr.nConstraints;  ++i)   {
		...
		assert_that( j < pr.ncourses,  "Invalid course for an unavailability constraint" );
		/* Found index `j' */
		...
		pr.course[j].unavail[ pr.course[j].nunavail - 1 ]  =  d * pr.hours  +  h;
	}
	...
	if ( argc >= 4  &&  strcmp(argv[3],"-fct") == 0 )   {
		...
		while ( resultsFile >> str >> strRoom >> d >> h )   {
			...
			assert_that( ++pr.course[i].lecturesScheduled <= pr.course[i].lectures,
					"Too many lectures scheduled" );

			//assert_that( resultsFile >> str,  "No room for this lecture" );
			...
			assert_that( j < pr.nrooms,  "Invalid room for this lecture" );
			/* Found index `j' */

			pr.course[i].roomScheduled[ pr.course[i].lecturesScheduled - 1 ]  =  j;

			assert_that( //resultsFile >> d >> h &&
					0 <= d  &&  d < pr.days  &&  0 <= h  &&  h < pr.hours,
					"Invalid lecture period" );

			pr.course[i].periodScheduled[ pr.course[i].lecturesScheduled - 1 ]  =  d * pr.hours  +  h;
		}

		assert_that( resultsFile.eof()  ||  str == "Cost",  "Wrong lecture description" );
	...
	for (currentRank=0, i=0;  i < pr.nrooms;  ++i)   {
		...
		if ( i == 0  ||  allRooms[i].room->capacity < allRooms[i-1].room->capacity )
			currentRank = i;
		else
			assert_that( allRooms[i].room->capacity == allRooms[i-1].room->capacity ,  "Wrong rank" );

		allRooms[i].room->rank = currentRank;

		//cout << "allRooms[" << i << "].rank= " << allRooms[i].room->rank
	...
	//		/*VOID*/;
	//	assert_that( static_cast<unsigned>(j) < roomCapacity.size(),  "Wrong rank" );
	//	pr.room[i].rank = j;
	//}
	//
	//roomCapacity.clear();
	...
	if ( str != "" )   {
		...
		for ( ; ; )   {
			...
			if ( str == "searchMethod" )   {
					...
					assert_that( optionsFile >> str ,  "Missing value for \"searchMethod\"" );
					...
					if ( str == "DFS" )   {
						...
						pr.searchMethodParameter[3] = atoi( str.c_str() );
						...
						assert_that( ( optionsFile >> pr.searchMethodParameter[0] ) ,
							   "Missing/Wrong parameter for search method \"CREDIT\"" );
						...
						assert_that( ( optionsFile >> pr.searchMethodParameter[0] ) ,
							   "Missing/Wrong parameter for search method \"LAN\"" );
						...
						assert_that( ( optionsFile >> pr.searchMethodParameter[0] )
								&& ( optionsFile >> pr.searchMethodParameter[1] ) ,
							   "Missing/Wrong parameter(s) for search method \"RDFS\"" );
						...
						assert_that( ( optionsFile >> str ) ,
							   "Missing parameter for search method \"FNS\"" );

						if ( str == "fSample" )
							pr.searchMethodParameter[0] = FSAMPLE;
						else if ( str == "fSampleMiddle" )
							pr.searchMethodParameter[0] = FSAMPLE_MIDDLE;
						else if ( str == "fSampleExp" )
							pr.searchMethodParameter[0] = FSAMPLE_EXP;
						else
							throw  timetableException("Missing/Wrong parameter for search method \"FNS\"");
						...
						assert_that( ( optionsFile >> pr.searchMethodParameter[0] ) ,
							   "Missing/Wrong parameter for search method \"ISAMP\"" );
						...
						throw  timetableException("Unknown \"searchMethod\" in options-file");
					}
					...
					assert_that( optionsFile >> str ,  "Missing value for \"metaSearchMethod\"" );
					...
					if ( str == "NONE" )   {
						...
						pr.searchMethodParameter_ls[3] = atoi( str.c_str() );
						...
						assert_that( ( optionsFile >> pr.searchMethodParameter_ls[0] )
								&& ( optionsFile >> pr.searchMethodParameter_ls[1] ) ,
							   "Missing/Wrong parameter(s) for search method \"RDFS\"" );
						...
						if ( str == "fSample" )
							pr.searchMethodParameter_ls[0] = FSAMPLE;
						else if ( str == "fSampleMiddle" )
							pr.searchMethodParameter_ls[0] = FSAMPLE_MIDDLE;
						else if ( str == "fSampleExp" )
							pr.searchMethodParameter_ls[0] = FSAMPLE_EXP;
						else
							throw  timetableException("Missing/Wrong parameter for search method \"FNS\"");
						...
						throw  timetableException("Unknown \"metaSearchMethod\" in options-file");
					}
					...
					assert_that( optionsFile >> pr.timeLimitDirectMethodRound ,
							"Missing value for \"timeLimitDirectMethodRound\"" );

			} else if ( str == "timeLimitIndirectMethodRound" )   {

					assert_that( optionsFile >> pr.timeLimitIndirectMethodRound ,
							"Missing value for \"timeLimitIndirectMethodRound\"" );

			} else if ( str == "quiet" )   {

					pr.quiet  =  true;

//#   If `timeType' is `CPU', then the `timeLimit' will be treated as       #
//#    CPU time.                                                            #
//#                                                                         #
//#      timeType  [REAL|CPU]                                               #
//#                                                                         #
...
//#                                                                         #
			...
			//
			//		assert_that( optionsFile >> str ,
			//				"Missing value for \"timeType\"" );
			//		if ( str == "REAL" )
			//			pr.isCpuTime = false;
			//		else if ( str == "CPU" )
			//			pr.isCpuTime = true;
			//		else
			//			throw  timetableException("Wrong value for \"timeType\"");


			} else if ( str == "constrainingFactorExponent" )   {

					assert_that( optionsFile >> pr.constrainingFactorExponent ,
							"Missing value for \"constrainingFactorExponent\"" );
			...
			} else if ( str == "leastConstrainingValueHeuristicWeight" )   {

					assert_that( optionsFile >> pr.leastConstrainingValueHeuristicWeight ,
							"Missing value for \"leastConstrainingValueHeuristicWeight\"" );


			} else if ( str == "objectiveOrientedValueHeuristicWeight" )   {

					assert_that( optionsFile >> pr.objectiveOrientedValueHeuristicWeight ,
							"Missing value for \"objectiveOrientedValueHeuristicWeight\"" );


			} else   {
					throw  timetableException("Unknown option in options-file");
			}
		}
	...
	void
writeSolution (const struct itcProblem_t& pr, NsIntVarArray& vLectPeriod, NsIntVarArray& vLectRoom,
...
/* Writing to solution file */
{
	...
	for (k=0,i=0;  i < pr.ncourses;  ++i)   {
		...
		for (j=0;  j < pr.course[i].lectures;  ++j,++k)   {
			...
			pr.course[i].periodScheduled[ pr.course[i].lecturesScheduled-1 ] =
				vLectPeriod[k].value();
			pr.course[i].roomScheduled[   pr.course[i].lecturesScheduled-1 ] =
				vLectRoom[k].value();
		}
	}
	...
	void
destroyInstance (const struct itcProblem_t& pr)
/* Memory de-allocation */
{
	...
	void
replaceQuote (char *str, ofstream& os)
/* Used for "escaping" quotes of a string written to a file. */
{
	...
	void
createFctResultsFile (const struct itcProblem_t& pr, string resultsFileName)
{
	int  i, j, k;
	...
	for (i=1;  i <= pr.ncourses;  ++i)   {
		...
		for (j=0;  j < pr.course[i-1].lecturesScheduled;  ++j)   {
			fctFile << pr.course[i-1].periodScheduled[j] << " " << pr.course[i-1].roomScheduled[j]+1;
			if ( j < pr.course[i-1].lecturesScheduled - 1 )
				fctFile << " ";
		}
		fctFile << "],[";
		for (j=0;  j < pr.course[i-1].lecturesScheduled;  ++j)   {
			/* Searching if `roomScheduled[j]' has been already recorded */
			for (k=0;  k < j;  ++k)   {
				if ( pr.course[i-1].roomScheduled[k] == pr.course[i-1].roomScheduled[j] )
					break;
			}
			if ( k == j )   {
				...
				if ( j > 0 )
					fctFile << " ";
				fctFile << pr.course[i-1].roomScheduled[j]+1;
			}
		}
		fctFile << "],'C" << i <<"',[";
		for (j=0;  j < pr.course[i-1].nunavail;  ++j)  {
			fctFile << "na(" << (pr.course[i-1].unavail[j])/pr.hours+1 << ","
	...
	for (i=1;  i <= pr.ngroups && static_cast<unsigned>(i) <= sizeof(colorCode)/sizeof(colorCode[0]);  ++i)   {
	...
	//cerr << "Remove its first line for more compatibility with fct format.\n";
}
