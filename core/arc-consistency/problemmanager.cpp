...
// Strictly speaking, in AC-3, arc (i, j) is not enqueued when arc (j, i) is made consistent. This optimization
// could be added in AC-5 by adding j as an argument to Enqueue and adding the constraint k != j to its
// definition.
NsConstraint*
QueueItem::getNextConstraint (void)
{
	/*
...
NsProblemManager::~NsProblemManager (void)
{
	for (CONSTR_arr_t::iterator c = CONSTR.begin();  c != CONSTR.end();  ++c)
		delete *c;
	for (NsDeque<NsIntVar*>::iterator v = IntermediateVar.begin();  v != IntermediateVar.end();  ++v)
		delete *v;
}
...
void
NsProblemManager::add (const ExprConstr& expr)
{
	assert_exc( FirstNextSolution ,
...
//{
//	//assert_exc( !initialized ,  "NsProblemManager::add_XlessthanY: Search has been initialized");
//
...
//{
//	//assert_exc( !initialized ,  "NsProblemManager::add_XeqYplusC: Search has been initialized");
//
...
//
//void
//NsProblemManager::add_XeqYplusCZspecial (NsIntVar& X, NsIntVar& Y, const NsInt C, NsIntVar& Z)
//{
//	//assert_exc( !initialized ,  "NsProblemManager::add_XeqYplusCZspecial: Search has been initialized");
//
...
//{
//	//assert_exc( !initialized ,  "NsProblemManager::add_XeqYdivC: Search has been initialized");
//
...
//{
//	//assert_exc( !initialized ,  "NsProblemManager::add_XneqY: Search has been initialized");
//
...
//{
//	assert( VarGoal != 0 );
//	assert_exc( !initialized ,  "NsProblemManager::Initialize: Search has been already initialized");
//	initialized      = true;
...
//		return  false;
//	}
//	//searchNodes.pop();                  // ... because there would be no backtracking
...
bool
NsProblemManager::ArcConsinstent (void)
{
	//assert_exc(initialized, "NsProblemManager::ArcConsinstent: Search has not been initialized");
	...
	while  ( !Q.empty() )   {
		assert_exc( !Q.top().getVarFired()->contains( Q.top().getW() ),
				"NsProblemManager::ArcConsinstent: A variable contains a removed value");
...
NsProblemManager::BackTrack (void)
{
	...
	for (;;)   {
		...
		assert_exc( !searchNodes.empty(), "NsProblemManager::BackTrack: `searchNodes' is empty");

		GoalNextChoice = searchNodes.top().GoalNextChoice;
		if ( GoalNextChoice == 0 )
			return  false;

		StackDomains&  DomsOriginal = searchNodes.top().DomsStore.DomsOrig;
		while ( !DomsOriginal.empty() )   {
			DomsOriginal.top().Var->setDomain( DomsOriginal.top().Dom );
			DomsOriginal.pop();
		}
...
NsProblemManager::restart (void)
{
	...
	assert_exc( !searchNodes.empty(), "NsProblemManager::restart: `searchNodes' is empty");
	...
	do   {
		...
		assert_exc( !searchNodes.empty(), "`restart()' call, before `nextSolution()'");
...
NsProblemManager::nextSolution (void)
{
	...
	if ( FirstNextSolution )   {
		...
		//  We took care placing it *after* the ArcConsinstent() call (because
		...
		if ( searchNodes.top().StackAND.empty()  &&  searchNodes.top().delayed_goal == searchNodes.gend() )
			return  IsArcCons;
	}
	...
	if ( CalledTimeLimit && timeLim != 0 )   {
		...
		if ( isRealTime )   {
			...
			assert_exc(startRealTime != -1,  "Could not find time for `realTimeLimit'");
		} else  {
			startTime = clock();
			assert_exc(startTime != -1,  "Could not find time for `timeLimit'");
		}
	}
	                       ...
	                       ||  ( !isRealTime  &&  static_cast<unsigned long>(clock()-startTime) <= timeLim*static_cast<unsigned long>(CLOCKS_PER_SEC) ) )
	{
		...
		if ( !searchNodes.top().StackAND.empty() )   {
			...
			assert_exc(searchNodes.top().delayed_goal != searchNodes.gend(),
					"NsProblemManager::nextSolution: No goal to execute");
			CurrGoal = *searchNodes.top().delayed_goal;
			++searchNodes.top().delayed_goal;
		}

		assert_exc(CurrGoal != 0,  "NsProblemManager::nextSolution: Zero goal to execute");
		...
		if ( CurrGoal->isGoalAND() )   {
			...
			searchNodes.push( SearchNode( CurrGoal->getSecondSubGoal(), searchNodes.gbegin() ) );
			searchNodes.top().StackAND.push( CurrGoal->getFirstSubGoal() );
			if (poppedAgoal)
				delete  CurrGoal;
			//cout << "--- OR  ---\n";
			...
			{
				if ( VarMinObj != 0 )   {
					assert_exc(BestMinObj > VarMinObj->max(),
					...
					//   where the domain of VarMinObj has been augmented.
				}
