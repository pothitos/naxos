...
/* Abstract class to represent an (algebraic etc) expression consisting of constrained variables */
...
class  Expression  {
	...
	public:
		...
		//  produces/returns a variable to represent the Expression (`*this')
		  ...
		  //{  throw  NsException("Expression::post: this is an \"abstract\" function");  }
...
class  NsDeque : public std::deque<TemplType>  {
	...
	public:
		...
		Ns_ExprElement
		operator  [] (const Expression& VarIndexExpr)  const
		{
...
inline void
assert_exc (const bool test, const char* error_message)
...
inline void
assert_Ns (const bool test, const char* error_message)
{
...
class  IntDomain  {
	...
	private:
		...
		NsDeque<unsigned>  machw;		// An array consisting of machine words.
	...
	private:
		...
		{
			if ( min_dom < 0  &&  val > 0 )	// to avoid an overflow
	...
	public:
		...
		bool  isContinuous (void)  const
		{
			...
			if (min_val < 0  &&  max_val > 0)	// to avoid an overflow
		...
		class  const_iterator  {
			...
			public:
				...
				{
					assert_exc(IntDom != 0,
							"IntDomain::const_iterator::==: Uninitialized `*this'");
				...
				NsInt  operator * (void)  const
				{
					...
					assert_exc(curr_val != NsPLUS_INF,
						"IntDomain::const_iterator::*: Bad request `*(something.end())'");
				...
				{
					assert_exc(IntDom != 0,
							"IntDomain::const_iterator::end: Uninitialized `*this'");
		...
		//  Declaration of an iterator to iterate through all the values
		//   of the domain, without changing them, in reverse order (const_iterator).
		...
		class  const_reverse_iterator  {
			...
			public:
				...
				{
					assert_exc(IntDom != 0,
							"IntDomain::const_reverse_iterator::==: Uninitialized `*this'");
				...
				NsInt  operator * (void)  const
				{
					...
					assert_exc(curr_val != NsPLUS_INF,
						"IntDomain::const_reverse_iterator::*: Bad request `*(something.end())'");
				...
				{
					assert_exc(IntDom != 0,
							"IntDomain::const_reverse_iterator::end: Uninitialized `*this'");
		...
		//  Declaration of an iterator to iterate through all the missing values (gaps)
		//   of the domain [min_val .. max_val], without changing them (const_iterator).
		...
		class  const_gap_iterator  {
			...
			public:
				...
				const_gap_iterator (const IntDomain&  IntDom_init)
					: IntDom(&IntDom_init), curr_gap(IntDom->next_gap(IntDom->min_val))
				{    }
				...
				{
					assert_exc(IntDom != 0,
							"IntDomain::const_gap_iterator::==: Uninitialized `*this'");
				...
				NsInt  operator * (void)  const
				{
					...
					assert_exc(curr_gap != NsPLUS_INF,
						"IntDomain::const_gap_iterator::*: Bad request `*(something.end())'");
				...
				{
					assert_exc(IntDom != 0,
							"IntDomain::const_gap_iterator::end: Uninitialized `*this'");
		...
		void  reset (NsInt val);	// Removes a value from the domain


			NsUInt
		size (void)  const		// It returns the domain size
		{
			return  set_count;
		}
		...
		// It returns the biggest value in the domain that is less than `to_val'
			NsInt
		previous (NsInt to_val)  const;


		// It returns the smaller value in the domain that is greater than `to_val'
			NsInt
		next (NsInt to_val)  const;


		// It returns the smaller value *not* in the domain that is greater than `to_val'
			NsInt
		next_gap (NsInt to_val)  const;
		...
		min (void)  const
		{
			...
			bool
		operator  [] (const NsInt val)  const
		{
			return  contains(val);
		}
		...
		//  The following method returns `true' if the domain is represented only
		//   by its lower and upper limit (this is the case when it is continuous).
			bool
		boundsConsistent (void)  const
		{
			return  ( machw.size()  ==  0 );
		}
...
class  NsIntVar  {
	...
	private:
		...
		NsProblemManager  *pm;		// Each constrained variable belongs to a specific "problem manager"
	...
	public:
			...
			void
		setDomain (const IntDomain& domain)
		{
			dom = domain;
		}
		...
		NsIntVar (void)
			: pm(0), ArcsConnectedTo(0)/*, id(0)*/
		{   }


		NsIntVar (NsProblemManager& pm_init, const NsInt min_dom_init, const NsInt max_dom_init);
		...
		void  removeAll (void);		// To remove in purpose all the values of the
		...
		bool  removeRange (NsInt val_start, NsInt val_end, const NsConstraint *c=0);
		...
		size (void)  const
		{
			...
			NsInt
		previous (const NsInt to_val)  const
		{
		...
		// If the constrained variable is instantiated, the following returns its value
			NsInt
		value (void)  const
		{
			assert_exc(isBound(),  "NsIntVar::value: `*this': Not a bound NsIntVar");
	...
	public:
		...
		const CONSTR_arr_t::const_iterator
		CONSTRbegin (void)  const
		{
			return  CONSTR.begin();
		}
...
/* Class describing a flexible array data type, to hold constrained variables (`NsIntVar's) */
...
class  NsIntVarArray  {
	...
	public:
		...
		//NsIntVarArray (const NsIndex length) : PointArray(length), added_constraint(false)
		//	{    }
		...
		//{
		//	assert_exc( empty()  &&  !added_constraint,  "NsIntVarArray::push_front: Cannot resize, because it is not empty");
	...
	//private:
		...
		{
			assert_exc( !added_constraint, "NsIntVarArray::push_front: Cannot add another variable, because a constraint has been already imposed on the array");
	...
	//private:
		...
		{
			assert_exc( !added_constraint, "NsIntVarArray::push_back: Cannot add another variable, because a constraint has been already imposed on the array");
...
class ConstrXlessthanY : public NsConstraint  {
	...
	public:
		...
		ConstrXlessthanY (NsIntVar *X, NsIntVar *Y)
		 : VarX(X), VarY(Y)
		{
			assert_exc( &VarX->manager() == &VarY->manager(),  "ConstrXlessthanY::ConstrXlessthanY: All the variables of a constraint must belong to the same NsProblemManager");
		}
...
class ConstrXlesseqthanY : public NsConstraint  {
	...
	public:
		...
		ConstrXlesseqthanY (NsIntVar *X, NsIntVar *Y)
		 : VarX(X), VarY(Y)
		{
			assert_exc( &VarX->manager() == &VarY->manager(),  "ConstrXlesseqthanY::ConstrXlesseqthanY: All the variables of a constraint must belong to the same NsProblemManager");
		}
...
class ConstrXeqYplusC : public NsConstraint  {
	...
	public:
		...
		{
			assert_exc( &VarX->manager() == &VarY->manager(),  "ConstrXeqYplusC::ConstrXeqYplusC: All the variables of a constraint must belong to the same NsProblemManager");
		}
...
class ConstrXeqCminusY : public NsConstraint  {
	...
	public:
		...
		ConstrXeqCminusY (NsIntVar *X, const NsInt C_init, NsIntVar *Y)
		 : VarX(X), VarY(Y), C(C_init)
		{
			assert_exc( &VarX->manager() == &VarY->manager(),  "ConstrXeqCminusY::ConstrXeqCminusY: All the variables of a constraint must belong to the same NsProblemManager");
		}
...
class ConstrXeqYtimesC : public NsConstraint  {
	...
	public:
		...
		ConstrXeqYtimesC (NsIntVar *X, NsIntVar *Y, const NsInt C_init)
		 : VarX(X), VarY(Y), C(C_init)
		{
			assert_exc( &VarX->manager() == &VarY->manager(),  "ConstrXeqYtimesC::ConstrXeqYtimesC: All the variables of a constraint must belong to the same NsProblemManager");
			assert_exc( C != 0,  "ConstrXeqYtimesC::ConstrXeqYtimesC: If C==0 the constraint has no reason to exist");
		}
...
class ConstrXeqYplusCZspecial : public NsConstraint  {
	...
	public:
		...
		ConstrXeqYplusCZspecial (NsIntVar *X, NsIntVar *Y, const NsInt C_init, NsIntVar *Z)
		 : VarX(X), VarY(Y), VarZ(Z), C(C_init)
		{
			assert_exc( &VarX->manager() == &VarY->manager()  &&  &VarY->manager() == &VarZ->manager(),  "ConstrXeqYplusCZspecial::ConstrXeqYplusCZspecial: All the variables of a constraint must belong to the same NsProblemManager");

			assert_exc(X->min() >= 0,
				"ConstrXeqYplusCZspecial::ConstrXeqYplusCZspecial: Special condition required:  X >= 0");
			assert_exc(0 <= Y->min()  &&  Y->max() < C,
				"ConstrXeqYplusCZspecial::ConstrXeqYplusCZspecial: Special condition required:  0 <= Y < C");
			assert_exc(C > 0,
				"ConstrXeqYplusCZspecial::ConstrXeqYplusCZspecial: Condition required:  C > 0");
		}
...
class ConstrXeqYplusZ: public NsConstraint  {
	...
	public:
		...
		{
			assert_exc( &VarX->manager() == &VarY->manager()  &&  &VarY->manager() == &VarZ->manager(),  "ConstrXeqYplusZ::ConstrXeqYplusZ: All the variables of a constraint must belong to the same NsProblemManager");
		}
...
class ConstrXeqYtimesZ: public NsConstraint  {
	...
	public:
		...
		ConstrXeqYtimesZ (NsIntVar *X, NsIntVar *Y, NsIntVar *Z)
		 : VarX(X), VarY(Y), VarZ(Z)
		{
			assert_exc( &VarX->manager() == &VarY->manager()  &&  &VarY->manager() == &VarZ->manager(),  "ConstrXeqYtimesZ::ConstrXeqYtimesZ: All the variables of a constraint must belong to the same NsProblemManager");
		}
...
class ConstrXeqYdivC : public NsConstraint  {
	...
	public:
		...
		ConstrXeqYdivC (NsIntVar *X, NsIntVar *Y, const NsInt C_init)
		 : VarX(X), VarY(Y), C(C_init)
		{
			assert_exc( &VarX->manager() == &VarY->manager(),  "ConstrXeqYdivC::ConstrXeqYdivC: All the variables of a constraint must belong to the same NsProblemManager");
			//assert_exc(Y->min() >= 0,
			//		"ConstrXeqYdivC::ConstrXeqYdivC: Special condition required:  Y >= 0");
			assert_exc(C > 0,
					"ConstrXeqYdivC::ConstrXeqYdivC: Special condition required:  C > 0");
		}
...
class ConstrXeqCdivY : public NsConstraint  {
	...
	public:
		...
		ConstrXeqCdivY (NsIntVar *X, const NsInt C_init, NsIntVar *Y)
		 : VarX(X), VarY(Y), C(C_init)
		{
			assert_exc( &VarX->manager() == &VarY->manager(),  "ConstrXeqCdivY::ConstrXeqCdivY: All the variables of a constraint must belong to the same NsProblemManager");
			assert_exc(C > 0,
				"ConstrXeqCdivY::ConstrXeqCdivY: Special condition required:  C > 0");
			if (VarY->contains(0))
...
class ConstrXeqYmodC : public NsConstraint  {
	...
	public:
		...
		ConstrXeqYmodC (NsIntVar *X, NsIntVar *Y, const NsInt C_init)
		 : VarX(X), VarY(Y), C(C_init)
		{
			assert_exc( &VarX->manager() == &VarY->manager(),  "ConstrXeqYmodC::ConstrXeqYmodC: All the variables of a constraint must belong to the same NsProblemManager");
			assert_exc(Y->min() >= 0,
					"ConstrXeqYmodC::ConstrXeqYmodC: Special condition required:  Y >= 0");
			assert_exc(C > 0,
					"ConstrXeqYmodC::ConstrXeqYmodC: Special condition required:  C > 0");
		}
...
class ConstrMetaXeqYlessthanC : public NsConstraint  {
	...
	public:
		...
		ConstrMetaXeqYlessthanC (NsIntVar *X, NsIntVar *Y, const NsInt C_init)
		 : VarX(X), VarY(Y), C(C_init)
		{
			assert_exc( &VarX->manager() == &VarY->manager(),  "ConstrMetaXeqYlessthanC::ConstrMetaXeqYlessthanC: All the variables of a constraint must belong to the same NsProblemManager");
		}
...
class ConstrMetaXeqYlessthanZ : public NsConstraint  {
	...
	public:
		...
		ConstrMetaXeqYlessthanZ (NsIntVar *X, NsIntVar *Y, NsIntVar *Z)
		 : VarX(X), VarY(Y), VarZ(Z)
		{
			assert_exc( &VarX->manager() == &VarY->manager()  &&  &VarY->manager() == &VarZ->manager(),  "ConstrMetaXeqYlessthanZ::ConstrMetaXeqYlessthanZ: All the variables of a constraint must belong to the same NsProblemManager");
		}
...
class ConstrMetaXeqYlesseqthanC : public NsConstraint  {
	...
	public:
		...
		ConstrMetaXeqYlesseqthanC (NsIntVar *X, NsIntVar *Y, const NsInt C_init)
		 : VarX(X), VarY(Y), C(C_init)
		{
			assert_exc( &VarX->manager() == &VarY->manager(),  "ConstrMetaXeqYlesseqthanC::ConstrMetaXeqYlesseqthanC: All the variables of a constraint must belong to the same NsProblemManager");
		}
...
class ConstrMetaXeqYlesseqthanZ : public NsConstraint  {
	...
	public:
		...
		ConstrMetaXeqYlesseqthanZ (NsIntVar *X, NsIntVar *Y, NsIntVar *Z)
		 : VarX(X), VarY(Y), VarZ(Z)
		{
			assert_exc( &VarX->manager() == &VarY->manager()  &&  &VarY->manager() == &VarZ->manager(),  "ConstrMetaXeqYlesseqthanZ::ConstrMetaXeqYlesseqthanZ: All the variables of a constraint must belong to the same NsProblemManager");
		}
...
class ConstrMetaXeqYgreaterthanC : public NsConstraint  {
	...
	public:
		...
		ConstrMetaXeqYgreaterthanC (NsIntVar *X, NsIntVar *Y, const NsInt C_init)
		 : VarX(X), VarY(Y), C(C_init)
		{
			assert_exc( &VarX->manager() == &VarY->manager(),  "ConstrMetaXeqYgreaterthanC::ConstrMetaXeqYgreaterthanC: All the variables of a constraint must belong to the same NsProblemManager");
		}
...
class ConstrMetaXeqYgreatereqthanC : public NsConstraint  {
	...
	public:
		...
		ConstrMetaXeqYgreatereqthanC (NsIntVar *X, NsIntVar *Y, const NsInt C_init)
		 : VarX(X), VarY(Y), C(C_init)
		{
			assert_exc( &VarX->manager() == &VarY->manager(),  "ConstrMetaXeqYgreatereqthanC::ConstrMetaXeqYgreatereqthanC: All the variables of a constraint must belong to the same NsProblemManager");
		}
...
class ConstrMetaXeqYeqC : public NsConstraint  {
	...
	public:
		...
		ConstrMetaXeqYeqC (NsIntVar *X, NsIntVar *Y, const NsInt C_init)
		 : VarX(X), VarY(Y), C(C_init)
		{
			assert_exc( &VarX->manager() == &VarY->manager(),  "ConstrMetaXeqYeqC::ConstrMetaXeqYeqC: All the variables of a constraint must belong to the same NsProblemManager");
		}
...
class ConstrMetaXeqYeqZ : public NsConstraint  {
	...
	public:
		...
		{
			assert_exc( &VarX->manager() == &VarY->manager()  &&  &VarY->manager() == &VarZ->manager(),  "ConstrMetaXeqYeqZ::ConstrMetaXeqYeqZ: All the variables of a constraint must belong to the same NsProblemManager");
		}
...
class ConstrMetaXeqYneqC : public NsConstraint  {
	...
	public:
		...
		ConstrMetaXeqYneqC (NsIntVar *X, NsIntVar *Y, const NsInt C_init)
		 : VarX(X), VarY(Y), C(C_init)
		{
			assert_exc( &VarX->manager() == &VarY->manager(),  "ConstrMetaXeqYneqC::ConstrMetaXeqYneqC: All the variables of a constraint must belong to the same NsProblemManager");
		}
...
//			: VarX(X), VarY(Y), VarZ(Z)
//		{
//			assert_exc( VarX->manager() == VarY->manager()  &&  VarY->manager() == VarZ->manager(),  "ConstrMetaXeqYneqZ::ConstrMetaXeqYneqZ: All the variables of a constraint must belong to the same NsProblemManager");
//		}
//
//		virtual int   VarsInvolvedIn (void)  const    {  return 3;  }
//
...
class ConstrXeqYandZ : public NsConstraint  {
	...
	public:
		...
		ConstrXeqYandZ (NsIntVar *X, NsIntVar *Y, NsIntVar *Z, const bool pos)
		 : VarX(X), VarY(Y), VarZ(Z), neg(!pos)
		{
			assert_exc( &VarX->manager() == &VarY->manager()  &&  &VarY->manager() == &VarZ->manager(),  "ConstrXeqYandZ::ConstrXeqYandZ: All the variables of a constraint must belong to the same NsProblemManager");
			assert_exc( 0 <= VarX->min() && VarX->max() <= 1  &&  0 <= VarY->min() && VarY->max() <= 1  &&  0 <= VarZ->min() && VarZ->max() <= 1,  "ConstrXeqYandZ::ConstrXeqYandZ: All the variables should be \"boolean\"");
		}
...
class ConstrXeqYorZ : public NsConstraint  {
	...
	public:
		...
		ConstrXeqYorZ (NsIntVar *X, NsIntVar *Y, NsIntVar *Z, const bool pos)
		 : VarX(X), VarY(Y), VarZ(Z), neg(!pos)
		{
			assert_exc( &VarX->manager() == &VarY->manager()  &&  &VarY->manager() == &VarZ->manager(),  "ConstrXeqYorZ::ConstrXeqYorZ: All the variables of a constraint must belong to the same NsProblemManager");
			assert_exc( 0 <= VarX->min() && VarX->max() <= 1  &&  0 <= VarY->min() && VarY->max() <= 1  &&  0 <= VarZ->min() && VarZ->max() <= 1,  "ConstrXeqYorZ::ConstrXeqYorZ: All the variables should be \"boolean\"");
		}
...
class ConstrXorY : public NsConstraint  {
	...
	public:
		...
		{
			assert_exc( &VarX->manager() == &VarY->manager(),  "ConstrXorY::ConstrXorY: All the variables of a constraint must belong to the same NsProblemManager");
			assert_exc( 0 <= VarX->min() && VarX->max() <= 1  &&  0 <= VarY->min() && VarY->max() <= 1,  "ConstrXorY::ConstrXorY: All the variables should be \"boolean\"");
		}
...
class ConstrXeqY : public NsConstraint  {
	...
	public:
		...
		ConstrXeqY (NsIntVar *X, NsIntVar *Y)
		 : VarX(X), VarY(Y)
		{
			assert_exc( &VarX->manager() == &VarY->manager(),  "ConstrXeqY::ConstrXeqY: All the variables of a constraint must belong to the same NsProblemManager");
		}
...
class ConstrXneqY : public NsConstraint  {
	...
	public:
		...
		ConstrXneqY (NsIntVar *X, NsIntVar *Y)
		 : VarX(X), VarY(Y)
		{
			assert_exc( &VarX->manager() == &VarY->manager(),  "ConstrXneqY::ConstrXneqY: All the variables of a constraint must belong to the same NsProblemManager");
		}
...
class ConstrXeqAbsY : public NsConstraint  {
	...
	public:
		...
		ConstrXeqAbsY (NsIntVar *X, NsIntVar *Y)
		 : VarX(X), VarY(Y)
		{
			assert_exc( &VarX->manager() == &VarY->manager(),  "ConstrXeqAbsY::cononstrXeqAbsY: All the variables of a constraint must belong to the same NsProblemManager");
		}
	...
	public:
		...
		class  groupedNsIntVar  {
			...
			public:
				...
				{
					assert_exc( vGroup.contains(groupVal-1),  "groupedNsIntVar::setGroup: `groupVal-1' is not contained in`vGroup'" );
					assert_exc( vGroup.removeRange(NsMINUS_INF, groupVal-1),  "groupedNsIntVar::setGroup: Could not change group to `groupVal'" );
					assert_exc( group() == groupVal,  "groupedNsIntVar::setGroup: Not succesful change of group to `groupVal'" );
				}
...
class Ns_ConstrCount : public NsConstraint  {
	...
	private:
		...
		/////  vCount[i].min() contains the number of the occurrences of Values[i] in VarArr.
...
class ConstrInverse : public NsConstraint  {
	...
	private:
		...
		class  ArrayIndex_t  {
			public:
				...
				const NsIndex  index;
				ArrayIndex_t (const bool InInverse_init, const NsIndex index_init)
	...
	public:
		...
		ConstrInverse (NsIntVarArray *VarArrInv_init, NsIntVarArray *VarArr_init);
...
class Ns_ConstrElement : public NsConstraint  {
	...
	public:
			  ...
			  VarValue(VarValue_init)
		{
			...
			assert_exc( &VarIndex->manager() == &VarValue->manager(),  "Ns_ConstrElement::Ns_ConstrElement: All the variables of a constraint must belong to the same NsProblemManager");

			assert_exc( ! intArray.empty() ,  "Ns_ConstrElement::Ns_ConstrElement: Empty element array" );
		}
...
class ExprYplusC : public Expression  {
	...
	public:
		...
		ExprYplusC (NsIntVar &Y, const NsInt C_init)
			: VarY(Y), C(C_init)	{   }
...
inline	ExprYplusC
operator +  (const Expression& Yexpr, const NsInt C)
{
	return  ( Yexpr.post() +  C );
}
...
inline	ExprYplusC
operator +  (const NsInt C, const Expression& Yexpr)
{
	return  ( Yexpr.post() +  C );
}
...
class ExprYdivC : public Expression  {
	...
	public:
		...
		ExprYdivC (NsIntVar &Y, const NsInt C_init)
			: VarY(Y), C(C_init)	{   }
...
class ExprYplusZ : public Expression  {
	...
	public:
		...
		ExprYplusZ (NsIntVar &Y, NsIntVar &Z)
			: VarY(Y), VarZ(Z)	{   }
...
inline	ExprYplusZ
operator +  (const Expression& Yexpr, NsIntVar& Z)
{
	return  ( Yexpr.post() + Z );
}

inline	ExprYplusZ
operator +  (NsIntVar& Y, const Expression& Zexpr)
{
	return  ( Y + Zexpr.post() );
}

inline	ExprYplusZ
operator +  (const Expression& Yexpr, const Expression& Zexpr)
{
...
class ExprYminusZ : public Expression  {
	...
	public:
		...
		ExprYminusZ (NsIntVar &Y, NsIntVar &Z)
			: VarY(Y), VarZ(Z)	{   }
...
class ExprCminusZ : public Expression  {
	...
	public:
		...
		ExprCminusZ (const NsInt C_init, NsIntVar &Z)
			: C(C_init), VarY(Z)	{   }
...
inline	ExprCminusZ
operator -  (const NsInt C, const Expression& Zexpr)
{
	return  ( C - Zexpr.post() );
}
...
class ExprYtimesZ : public Expression  {
	...
	public:
		...
		ExprYtimesZ (NsIntVar &Y, NsIntVar &Z)
			: VarY(Y), VarZ(Z)	{   }
...
class ExprYtimesC : public Expression  {
	...
	public:
		...
		ExprYtimesC (NsIntVar &Y, const NsInt C_init)
			: VarY(Y), C(C_init)	{   }
...
inline	ExprCminusZ
operator -  (const Expression& Yexpr)
{
	return  ( 0 - Yexpr.post() );
}
...
class ExprYdivZ : public Expression  {
	...
	public:
		...
		ExprYdivZ (NsIntVar &Y, NsIntVar &Z)
			: VarY(Y), VarZ(Z)	{   }
...
class ExprCdivZ : public Expression  {
	...
	public:
		...
		ExprCdivZ (const NsInt C_init, NsIntVar &Z)
			: C(C_init), VarZ(Z)	{   }
...
class ExprYmodZ : public Expression  {
	...
	public:
		...
		ExprYmodZ (NsIntVar &Y, NsIntVar &Z)
			: VarY(Y), VarZ(Z)	{   }
...
class ExprYmodC : public Expression  {
	...
	public:
		...
		ExprYmodC (NsIntVar &Y, const NsInt C_init)
			: VarY(Y), C(C_init)	{   }
...
class ExprCmodZ : public Expression  {
	...
	public:
		...
		ExprCmodZ (const NsInt C_init, NsIntVar &Z)
			: C(C_init), VarZ(Z)	{   }
...
class ExprInDomain : public Expression  {
	...
	public:
		...
		ExprInDomain (NsProblemManager& pm_init,
...
class ExprMin : public Expression  {
	...
	public:
		...
		ExprMin (NsIntVarArray  &VarArr_init)
			: VarArr(VarArr_init)	{   }
...
class ExprMax : public Expression  {
	...
	public:
		...
		ExprMax (NsIntVarArray  &VarArr_init)
			: VarArr(VarArr_init)	{   }
...
class ExprSum : public Expression  {
	...
	public:
		...
		{
			assert_exc(start+length <= VarArr.size(),  //&&  length >= 0,
...
/* Abstract class to represent an expression having to do with arrays of constrained variables */
...
class ExprConstr : public Expression  {
	...
	protected:
		...
		bool  is_positive;
		//  If `is_positive==false' then the semantics of the constraint-expression
	...
	public:
		...
		ExprConstr (const bool is_positive_init)
			: is_positive(is_positive_init)	{   }

		virtual NsConstraint*  postConstraint (bool positively=true)  const = 0;
		//	{  throw  NsException("ExprConstr::postConstraint: Unimplemented");  }
...
class ExprConstrYlessthanC : public ExprConstr  {
	...
	public:
		...
		ExprConstrYlessthanC (NsIntVar& Y, const NsInt C_init, const bool is_positive_init)
...
class ExprConstrYlesseqthanC : public ExprConstr  {
	...
	public:
		...
		ExprConstrYlesseqthanC (NsIntVar& Y, const NsInt C_init, const bool is_positive_init)
			: ExprConstr(is_positive_init), VarY(Y), C(C_init)	{   }
...
class ExprConstrYeqC : public ExprConstr  {
	...
	public:
		...
		ExprConstrYeqC (NsIntVar& Y, const NsInt C_init, const bool is_positive_init)
			: ExprConstr(is_positive_init), VarY(Y), C(C_init)	{   }
...
class ExprConstrYandZ : public ExprConstr  {
	...
	public:
		...
		ExprConstrYandZ (NsIntVar& Y, NsIntVar& Z)
			: ExprConstr(true), VarY(Y), VarZ(Z)
		{
			assert_exc(0 <= VarY.min()  &&  VarY.max() <= 1  &&  0 <= VarZ.min()  &&  VarZ.max() <= 1,
					"ExprConstrYandZ::ExprConstrYandZ: `VarY' and `VarZ' should be boolean");
		}

		virtual NsConstraint*  postConstraint (bool positively)  const
		{
			positively = positively;  // to suppress warnings
			throw  NsException("ExprConstrYandZ::postConstraint: Please, add the two constraints separately");
		}
...
class ExprConstrYorZ : public ExprConstr  {
	...
	public:
		...
		{
			assert_exc(0 <= VarY.min()  &&  VarY.max() <= 1  &&  0 <= VarZ.min()  &&  VarZ.max() <= 1,
					"ExprConstrYorZ::ExprConstrYorZ: `VarY' and `VarZ' should be boolean");
		}
...
class ExprConstrNegationY : public ExprConstr  {
	...
	public:
		...
		ExprConstrNegationY (const ExprConstr& Y)
			: ExprConstr(true), Yexpr(Y)	{    }
...
class ExprConstrAllDiff : public ExprConstr  {
	...
	public:
		...
		virtual void       postC (NsIntVar &VarX, bool positively)  const
		{
			VarX.size();              // to suppress warnings
			positively = positively;  // to suppress warnings
			throw  NsException("ExprConstrAllDiff::postC: NsAllDiff cannot be used as a meta-constraint");
		}
		virtual NsIntVar&  postC (bool positively)  const
		{
			positively = positively;  // to suppress warnings
...
inline	ExprConstrYlessthanZ
operator >  (NsIntVar& Z, const Expression& Yexpr)
{
	return  ( Yexpr  <  Z );
}

inline	ExprConstrYlessthanZ
operator >  (const Expression& Zexpr, NsIntVar& Y)
{
	return  ( Y  <  Zexpr );
}
...
inline	ExprConstrYlessthanZ
operator >  (const Expression& Zexpr, const Expression& Yexpr)
{
	return  ( Yexpr  <  Zexpr );
}
...
inline	ExprConstrYandZ
operator &&  (const ExprConstr& Yexpr, const ExprConstr& Zexpr)
{
	return  ExprConstrYandZ( Yexpr.post(), Zexpr.post() );
}


inline	ExprConstrYorZ
operator ||  (const ExprConstr& Yexpr, const ExprConstr& Zexpr)
{
...
inline	ExprConstrNegationY
operator !  (const ExprConstr& Yexpr)
{
...
inline	ExprConstrAllDiff
NsAllDiff  (NsIntVarArray& Arr, unsigned long Capacity=0)
{
...
NsCount  (NsIntVarArray& Arr, const NsDeque<NsInt>& Values, const NsDeque<NsInt>& Occurrences)
{
...
inline	ExprConstrYorZ
NsIfThen  (const ExprConstr& Yexpr, const ExprConstr& Zexpr)
{
...
inline	ExprConstrYeqZ
NsEquiv  (const ExprConstr& Yexpr, const ExprConstr& Zexpr)
{
	//  "p <=> q"  is equivalent to  "p == q".
	return  ( Yexpr  ==  Zexpr );
}
...
inline	ExprConstrYeqZ
NsElement  (const Expression& VarIndexExpr,
		const NsDeque<NsInt>& intArray,
		NsIntVar& VarValue)
{
...
/*
 *  `NsGoal' is an Abstract class representing an (internal or constructed by user) goal.
 *
 *  Each time a goal is executed by the solver, its method `GOAL()' is called.  This
 *   method can make assignments or remove values from constrained variables.
 */
...
class NsgAND : public NsGoal  {
	...
	public:
		...
		{
			assert_exc(FirstSubGoal!=0  &&  SecondSubGoal!=0,  "NsgAND::NsgAND: A subgoal is zero");
		}
...
class NsgOR : public NsGoal  {
	...
	public:
		...
		NsgOR (NsGoal *FirstSubGoal_init, NsGoal *SecondSubGoal_init)
		 : FirstSubGoal(FirstSubGoal_init), SecondSubGoal(SecondSubGoal_init)
		{
			assert_exc(FirstSubGoal!=0  &&  SecondSubGoal!=0,  "NsgOR::NsgOR: A subgoal is zero");
		}
...
//   It uses `NsgInDomain' to iteratively instantiate each variable of the array.
//   The next variable that is chosen is the one having the minimum domain size
//   (according to the "first-fail" heuristic).
...
class  QueueItem  {
	...
	public:
		...
		QueueItem (const NsIntVar *var_f, const NsConstraint *constr_f,
				const int w_init)
		 : var_fired(var_f), constr_fired(constr_f), w(w_init),
		   curr_constr(0)    {   }
...
//  `StackGoals' is a class normally used for describing the stack holding AND-goals
...
class Ns_StackSearch : public NsStack<SearchNode>  {
	...
	public:
		...
		//  (All of them consist a stack of `StackGoals', named `StackOfStacks'.)
		...
		class  goal_iterator  {
			...
			private:
				...
				Ns_StackSearch::iterator  curr_Stack_it;   // points to the current search node.
				StackGoals::iterator   curr_node_it;    // points to the current goal (in the
...
class  SearchNode  {
	...
	public:
		...
		SearchNode (NsGoal *GoalNextChoice_init, Ns_StackSearch::goal_iterator git)
			: GoalNextChoice(GoalNextChoice_init), delayed_goal(git),
		   has_a_child(false)
		{    }
...
class  NsProblemManager  {
	...
	private:
		...
		unsigned long  nFailures, nBackTracks, nGoals, nConstraintChecks, backTrackLim, nVars;


	public:

		NsProblemManager (void)
			:  VarMinObj(0), FoundInconsistency(false), timeLim(0),
			   FirstNextSolution(true), CalledTimeLimit(false), nFailures(0),
			   nBackTracks(0), nGoals(0), nConstraintChecks(0), backTrackLim(0), nVars(0)
			//, VarsID(0), VarGoal(0), initialized(false), finished(false),
		{
			assert_exc(sizeof(NsIntVar*) <= sizeof(pointer_t),
				"NsProblemManager::NsProblemManager: Cannot run on this machine, because a pointer does not fit in an `unsigned long' (the `unordered_set' type)");
			//searchNodes.push( SearchNode( 0, StackGoals::iterator(), searchNodes.gbegin() ) );
			searchNodes.push( SearchNode( 0, searchNodes.gbegin() ) );
			//searchNodes.push( SearchNode( 0, searchNodes.gbegin() ) );
			////  The first push takes place, because some goals (NsgSetValue and NsgRemoveValue)
			////   save the domains of the variables they handle, in the *previous* node of the
			////   search tree.
		}
		...
		FoundAnInconsistency (void)
		{
			...
			bool
		isPartiallyConsistent (void)  const
		{
		...
		//void  add_XeqYplusCZspecial (NsIntVar& X, NsIntVar& Y, const NsInt C, NsIntVar& Z);
		//void  add_XeqYdivC (NsIntVar& X, NsIntVar& Y, const NsInt C);
		//void  add_XneqY (NsIntVar& X, NsIntVar& Y);
		...
		{
			//assert_exc( !initialized ,  "NsProblemManager::minimize: Search has been initialized");
			VarMinObj = &VarObj;
		...
		{
			assert_exc( VarMinObj != 0,  "NsProblemManager::objectiveUpperLimit: No cost variable exists" );
		...
		{
			CalledTimeLimit = true;
			isRealTime = false;	// We use CPU time
			timeLim = lim;
		}
...
//		{
//			assert_exc(++VarsID != 0,  "NsProblemManager::generateID: `VarsID' overflow");
//			return  VarsID;
//		}
};
...
IntersectionEmpty (const NsIntVar *VarY, const NsIntVar *VarZ);
	...
	void
YmodC_min_max (const NsIntVar *VarY, const NsInt C, NsInt& min, NsInt& minmax);
...
array_maxmin_max (const NsIntVarArray *VarArr, NsInt& maxmin, NsInt& max);
	...
	void
array_sum_min_max (const NsIntVarArray *VarArr, const NsIndex start, const NsIndex length, NsInt& summin, NsInt& summax);
