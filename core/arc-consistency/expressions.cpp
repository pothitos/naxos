...
namespace  {
	...
	void
ExprYplusC::post (NsIntVar &VarX) const
{
...
ExprYplusC::post (void) const
{
	...
	void
ExprCminusZ::post (NsIntVar &VarX) const
{
...
ExprCminusZ::post (void) const
{
	...
	void
ExprYtimesC::post (NsIntVar &VarX) const
{
...
ExprYtimesC::post (void) const
{
	...
	void
ExprYdivC::post (NsIntVar &VarX) const
{
	VarX = NsIntVar(VarY.manager(), VarY.min() / C, VarY.max() / C);
...
ExprYdivC::post (void) const
{
	...
	void
ExprCdivZ::post (NsIntVar &VarX) const
{
	...
	NsIntVar&
ExprCdivZ::post (void) const
{
	NsIntVar  *VarX = new NsIntVar(VarZ.manager(), C / VarZ.max(), C / VarZ.min());
	...
	void
ExprYmodC::post (NsIntVar &VarX) const
{
	NsInt  min, max;
...
ExprYmodC::post (void) const
{
	...
	void
ExprAbsY::post (NsIntVar &VarX) const
{
...
namespace  {
	...
	void
ExprInDomain::post (NsIntVar &VarX)  const
{
	array_sort(domain);
...
namespace  {
	...
	{
		NsConstraint  *NewConstr = new ConstrXeqYplusCZspecial(&VarX, &VarY, C, &VarZ);
		VarX.addConstraint(NewConstr);
		VarY.addConstraint(NewConstr);
	...
	void
ExprYplusCZspecial::post (NsIntVar &VarX) const
{
	...
	NsIntVar&
ExprYplusCZspecial::post (void) const
{
...
namespace  {
	...
	void
ExprYplusZ::post (NsIntVar &VarX) const
{
...
ExprYplusZ::post (void) const
{
	...
	void
ExprYminusZ::post (NsIntVar &VarX) const
{
...
ExprYminusZ::post (void) const
{
	...
	void
ExprYtimesZ::post (NsIntVar &VarX) const
{
	NsInt  min, max;
...
ExprYtimesZ::post (void) const
{
	...
	void
ExprConstrYandZ::postC (NsIntVar &VarX, bool positively) const
{
	if ( !is_positive )
	...
	NsIntVar&
ExprConstrYandZ::postC (bool positively) const
{
	NsIntVar  *VarX;
	if ( !is_positive )
		positively = !positively;
	...
	void
ExprConstrYorZ::postC (NsIntVar &VarX, bool positively) const
{
	if ( !is_positive )
		positively = !positively;
	if (positively)  {
	...
	NsIntVar&
ExprConstrYorZ::postC (bool positively) const
{
	NsIntVar  *VarX;
	if ( !is_positive )
		positively = !positively;
	...
	NsConstraint*
ExprConstrYorZ::postConstraint (bool positively) const
{
	NsConstraint  *NewConstr;
	if ( !is_positive )
		positively = !positively;
...
namespace  {
	...
	{
		NsConstraint  *NewConstr = new ConstrXeqSum(&VarX, &VarArr, start, length);
	...
	void
ExprMin::post (NsIntVar &VarX) const
{
...
ExprMin::post (void) const
{
	...
	void
ExprMax::post (NsIntVar &VarX) const
{
...
ExprMax::post (void) const
{
	...
	void
ExprSum::post (NsIntVar &VarX) const
{
...
Ns_ExprElement::post (void)  const
{
	...
	void
ExprInverse::post (NsIntVarArray &VarArrInv) const
{
	assert_exc( VarArr.size() > 0 , "Cannot inverse an empty array" );

	assert_exc(VarArrInv.size() == 0,  "ExprInverse::post: `VarArrInv' non-empty");
	...
	if (MaxDom != -1)  {
		assert_exc(MaxDom >= max,  "ExprInverse::post: `MaxDom' is less than `VarArr' max");
		max = MaxDom;
	}
...
namespace  {
		void
	ExprConstrYopZ_post_constr (NsConstraint *NewConstr, NsIntVar &VarX, NsIntVar &VarY)
	{
		VarX.addConstraint(NewConstr);
		VarY.addConstraint(NewConstr);
		NewConstr->ArcCons();
		...
		void
	ExprConstrYopZ_post_constr (NsConstraint *NewConstr, NsIntVar &VarX, NsIntVar &VarY, NsIntVar &VarZ)
	{
		VarZ.addConstraint(NewConstr);
	...
	NsConstraint*
ExprConstrYlessthanC::postConstraint (bool positively) const
{
	if ( !is_positive )
		positively = !positively;
	if (positively)  {
	...
	void
ExprConstrYlessthanC::postC (NsIntVar &VarX, bool positively) const
{
	NsConstraint  *NewConstr;
	if ( !is_positive )
		positively = !positively;
	...
	NsIntVar&
ExprConstrYlessthanC::postC (bool positively) const
{
	NsIntVar  *VarX;
	NsConstraint  *NewConstr;
	if ( !is_positive )
	...
	NsConstraint*
ExprConstrYlesseqthanC::postConstraint (bool positively) const
{
	if ( !is_positive )
		positively = !positively;
	if (positively)  {
	...
	void
ExprConstrYlesseqthanC::postC (NsIntVar &VarX, bool positively) const
{
	NsConstraint  *NewConstr;
	if ( !is_positive )
		positively = !positively;
	...
	NsIntVar&
ExprConstrYlesseqthanC::postC (bool positively) const
{
	NsIntVar  *VarX;
	NsConstraint  *NewConstr;
	if ( !is_positive )
...
//	*VarX = NsIntVar(*VarY.manager(), VarY.min() > C, VarY.max() > C);
//
//	NsConstraint  *NewConstr = new ConstrMetaXeqYgreaterthanC(VarX, &VarY, C);
//	ExprConstrYopZ_post_constr(NewConstr, VarX, &VarY);
//}
//
//
...
//	*VarX = NsIntVar(*VarY.manager(), VarY.min() >= C, VarY.max() >= C);
//
//	NsConstraint  *NewConstr = new ConstrMetaXeqYgreatereqthanC(VarX, &VarY, C);
//	ExprConstrYopZ_post_constr(NewConstr, VarX, &VarY);
//}
//
//
...
//}
	...
	NsConstraint*
ExprConstrYeqC::postConstraint (bool positively) const
{
	if ( !is_positive )
		positively = !positively;
	if (positively)  {
	...
	void
ExprConstrYeqC::postC (NsIntVar &VarX, bool positively) const
{
	NsConstraint  *NewConstr;
	if ( !is_positive )
		positively = !positively;
	...
	NsIntVar&
ExprConstrYeqC::postC (bool positively) const
{
	NsIntVar  *VarX;
	NsConstraint  *NewConstr;
	if ( !is_positive )
...
//}
	...
	NsConstraint*
ExprConstrYlessthanZ::postConstraint (bool positively) const
{
	NsConstraint  *NewConstr;
	if ( !is_positive )
		positively = !positively;
	...
	void
ExprConstrYlessthanZ::postC (NsIntVar &VarX, bool positively) const
{
	NsConstraint  *NewConstr;
	if ( !is_positive )
		positively = !positively;
	...
	NsIntVar&
ExprConstrYlessthanZ::postC (bool positively) const
{
	NsIntVar  *VarX;
	NsConstraint  *NewConstr;
	if ( !is_positive )
	...
	NsConstraint*
ExprConstrYlesseqthanZ::postConstraint (bool positively) const
{
	NsConstraint  *NewConstr;
	if ( !is_positive )
		positively = !positively;
	...
	void
ExprConstrYlesseqthanZ::postC (NsIntVar &VarX, bool positively) const
{
	NsConstraint  *NewConstr;
	if ( !is_positive )
		positively = !positively;
	...
	NsIntVar&
ExprConstrYlesseqthanZ::postC (bool positively) const
{
	NsIntVar  *VarX;
	NsConstraint  *NewConstr;
	if ( !is_positive )
	...
	NsConstraint*
ExprConstrYeqZ::postConstraint (bool positively) const
{
	NsConstraint  *NewConstr;
	if ( !is_positive )
		positively = !positively;
	...
	void
ExprConstrYeqZ::postC (NsIntVar &VarX, bool positively) const
{
	NsConstraint  *NewConstr;
	if ( !is_positive )
		positively = !positively;
	...
	NsIntVar&
ExprConstrYeqZ::postC (bool positively) const
{
	NsIntVar  *VarX;
	NsConstraint  *NewConstr;
	if ( !is_positive )
...
//}
	...
	NsConstraint*
ExprConstrAllDiff::postConstraint (bool positively) const
{
	assert_exc(positively,  "ExprConstrAllDiff::postConstraint: `positively'==false");
	if (VarArr.size() <= Capacity  ||  ( Capacity==0 && VarArr.size() <= 1 ) )
		return  0;  // no constraint

	NsConstraint  *NewConstr;
	if ( Capacity  ==  0 )
		NewConstr = new ConstrAllDiff(&VarArr);	// default case
	else
...
{
	assert_exc(positively,  "Ns_ExprConstrCount::postConstraint: `positively'==false");

	NsConstraint  *newConstr = new Ns_ConstrCount(&VarArr,Values,Occurrences);
