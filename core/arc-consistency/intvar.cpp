...
IntDomain::IntDomain (const NsInt min_dom_init, const NsInt max_dom_init)
	: min_dom(min_dom_init),
	  ...
	  set_count(nbits)
{
	assert_exc(NsMINUS_INF < min_dom_init  &&  min_dom_init <= max_dom_init  &&  max_dom_init < NsPLUS_INF,
			"IntDomain::IntDomain: Domain out of range");
	//assert_exc(nbits > 0,
	//		"IntDomain::IntDomain: Request to construct an empty domain");
...
//			"IntDomain::set: this function is Bounds-Consistency-incompatible");
//	assert_exc(min_val <= val && val <= max_val && nbit < nbits  &&  mw < machw.size(),
...
IntDomain::reset (NsInt val)
{
	...
	NsUInt  mw = nbit/MW_BITS;
	assert_exc(machw.size() != 0,
			"IntDomain::re-set: this function is Bounds-Consistency-incompatible");
	assert_exc(min_val <= val && val <= max_val && nbit < nbits  &&  mw < machw.size(),
	...
	NsInt
IntDomain::previous (NsInt val)  const
...
// ενεργού bit μετά το nbit
{
	...
	for ( ;  mw>=0;  --mw)  {
		if (machw[mw] == 0)  {	// speedup by means of comparing the whole word
			...
			for (mwbit=1<<(MW_BITS-1);  mwbit!=0;  mwbit>>=1)  {
				if (machw[mw] & mwbit)
					return  (nbit + min_dom);
				--nbit;
			}
	...
	NsInt
IntDomain::next (NsInt val)  const
// επιστρέφει τον αριθμό τού επόμενου
// ενεργού bit μετά το nbit
{
	if (val >= max_val)
	...
	NsInt
IntDomain::next_gap (NsInt val)  const
// επιστρέφει τον αριθμό τού επόμενου
// inactive bit μετά το nbit
{
	if (val >= max_val-1  ||  isContinuous())	// || Bounds Consistency
		return  NsPLUS_INF;
	NsUInt  nbit;
	if (val < min_val)
...
//}
	...
	bool
IntDomain::contains (const NsInt val)  const
{
...
//}
	...
	ostream&
naxos::operator << (ostream& os, const IntDomain& dom)
{
	//o << bp.machw.size();
	////assert(bp.machw.size() >= 0);
	//if (bp.machw.size() > 0)  {
	//	for (NsUInt j=0;  j<bp.machw.size();  ++j)
	//		o << ' ' << hex << setw(bp.MW_BITS/4) << setfill('0') << bp.machw[j];
	//	o << ' ' << dec << bp.set_count;
	//}
	//return  o;
	...
	//	NsInt  previous=NsMINUS_INF;  // initialized to suppress warnings

	//	NsIntVar::const_iterator  v = dom.begin();
	//	if (v != dom.end())  {
	//		os << *v;
	//		previous = *v;
	//		++v;
...
NsIntVar::remove (const NsInt v)
{
	...
	bool
NsIntVar::removeRange (NsInt vstart, NsInt vend, const NsConstraint *constr)
{
	...
	if (vend > max())   {
		...
		while ( ( vend = previous(vend) )  >  vstart )
			assert_exc( removeSingle( vend, constr ) , "NsIntVar::removeRange: Problem removing `vend'");
	} else  {
		while ( ( vstart = next(vstart) )  <  vend )
			assert_exc( removeSingle( vstart, constr ) , "NsIntVar::removeRange: Problem removing `vstart'");
	}
	return  true;
}
	...
	bool
NsIntVar::removeSingle (const NsInt v, const NsConstraint *constr)
{
	if (size() == 1)   {
		pm->FoundAnInconsistency();
		return  false;
	...
	//if ( pm->getCurrDomsStore().AlreadySaved.count( (pointer_t)this )  ==  0 )   {
	//	pm->getCurrDomsStore().AlreadySaved.insert( (pointer_t)this );
	//	pm->getCurrDomsStore().DomsOrig.push( VarDom_t( this , dom ) );
	//}
...
//}
	...
	void
NsIntVar::set (const NsInt v_set)
{
	//assert_exc( contains(v_set),  "NsIntVar::set: `*this' does not contain the value to set");
	//assert_exc( !isBound(),  "NsIntVar::set: `*this' is already bound");
	//for (const_iterator v = begin();   v != end();   ++v)  {
	//	if (*v != v_set)
	//		assert_exc( remove( *v, 0 ),  "NsIntVar::set: Failed to remove a value in the domain");
	//}

	//  Bounds Consistency compatible set
	//while ( min() < v_set )
	//	assert_exc( removeSingle( min(), 0 ),  "NsIntVar::set: Failed to remove a value in the domain");
	//while ( max() > v_set )
	//	assert_exc( removeSingle( max(), 0 ),  "NsIntVar::set: Failed to remove a value in the domain");
...
//}
	...
	void
NsIntVar::addConstraint (NsConstraint* constr)
{
	CONSTR.push_back(constr);
	assert_exc(constr->VarsInvolvedIn() >= 1,
...
NsIntVar::NsIntVar (NsProblemManager& pm_init, const NsInt min_dom_init, const NsInt max_dom_init)
...
NsIntVar::NsIntVar (const Expression& expr)
	: ArcsConnectedTo(0)//, id(0)
{
	assert_exc(CONSTR.size() == 0  &&  ArcsConnectedTo == 0,
			"NsIntVar::NsIntVar(expr): Some constraints already imposed on `this'");
	expr.post(*this);
}
...
{
	assert_exc(pm == 0  &&  CONSTR.size() == 0  &&  ArcsConnectedTo == 0,
...
//	PointArray.push_back( VarPointAllocated( &mess.Expr->post(), true ) );
//	delete  mess.Expr;
//}
...
NsIntVarArray::push_front (const NsIntVar& Var)
{
	...
	void
NsIntVarArray::push_front (const Expression& expr)
{
	assert_exc( !added_constraint, "NsIntVarArray::push_front: Cannot add another variable, because a constraint has been already imposed on the array");
...
NsIntVarArray::push_back (const NsIntVar& Var)
{
	...
	void
NsIntVarArray::push_back (const Expression& expr)
{
	assert_exc( !added_constraint, "NsIntVarArray::push_back: Cannot add another variable, because a constraint has been already imposed on the array");
