...
class AmPopsLabeling : public NsGoal {
    ...
    public:
        AmPopsLabeling(NsIntVarArray& Vars_init,
                       const double proportion_init,
                       VariableHeuristic *varHeuristic = new VarHeurMRV,
                       ValueHeuristic *valHeuristic = new ValHeurFirst)
                : Vars(Vars_init), proportion(proportion_init),
                  varHeur(varHeuristic), valHeur(valHeuristic)
        {
                assert_Ns( 0.0 <= proportion && proportion <= 1.0 ,
                           "AmPopsLabeling::AmPopsLabeling: proportion must be between 0 and 1");
        }
        NsGoal  *GOAL (void);
};
...
class AmPopsInDomain : public NsGoal {
    private:
        ...
        ValueHeuristic *valHeur;
    public:
        AmPopsInDomain (NsIntVar& Var_init,
                        const double proportion_init,
                        ValueHeuristic *valHeuristic = new ValHeurFirst)
                : Var(Var_init),
