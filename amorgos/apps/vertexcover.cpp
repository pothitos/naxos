    ...
    try  {
		...
		while(1) {	// is there a vertex cover C with |C|=k ?
			...
			for(i=0; i<N; ++i)
				for(j=0; j<i; ++j)
					if(graph[i][j])
						pm.add( Nodes[i] == 1  ||  Nodes[j] == 1 );

			pm.add ( NsSum(Nodes) == k );

			// GOALS //

			pm.addGoal(new goalDfsLabeling(Nodes));
			...
			if (pm.nextSolution()){
				...
				cout << " was found for the following graph:" << endl << endl;
				...
				cout << endl << "It consists of the following nodes:";
				for(i=0; i<N; ++i)
					if( Nodes[i].value() )
						cout << " " << i;
				cout << endl;
