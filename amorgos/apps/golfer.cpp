			...
			for (j=0; j<nGroups; ++j)
				for (k=0; k<groupSize-1; ++k)
					for (l=groupSize-1; l>k; --l){
						...
						pm.add(varRows[i][j*groupSize+k] <
						       varRows[i][j*groupSize+l]);

						// save pairs
//						pairsFirst.push_back( varRows[i][j*groupSize+k] );
//						pairsSecond.push_back( varRows[i][j*groupSize+l] );

						pairs.push_back( varRows[i][j*groupSize+k] *nGolfers
						               + varRows[i][j*groupSize+l] );
					}
		...
		for (i=0;i<nDays;++i)
			for (j=1;j<nGroups;++j)
				pm.add(varRows[i][(j-1)*groupSize] < varRows[i][j*groupSize]);
...
/*
		...
		for (i=0; i<pairsFirst.size(); ++i)
			for (j=0; j<pairsFirst.size(); ++j)
				if (i!=j)
					pm.add( pairsFirst[i] != pairsFirst[j] ||
					        pairsSecond[i] != pairsSecond[j] );
//					pm.add( pairsFirst[i]*nGolfers + pairsSecond[i] !=
...
*/
		...
		pm.addGoal( new goalDfsLabeling(Vars) );
